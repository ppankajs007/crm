...
class Customerassign extends MX_Controller
{
  ...
  function assign(){
    ...
    if($this->input->post('custm_id') != '' ){
        $postdata = array(
                ...
                'leads_id'    => $this->input->post('assign_lead'),
                'created_by'  => $this->tank_auth->get_user_id()
        );
        ...
        if( $selectRow == false  ){
            if( !$customer_assign = $this->customer_model->add('pk_customer_leads', $postdata ) ){
                ...
                $this->session->set_flashdata('response_status', 'error');
                $this->session->set_flashdata('message', 'Leads already\'exits, Add another ');
        }
        redirect('/customer');
  ...
  function address_ads()
    ...
    function cust_lead(){
        ...
        // $selectRow = (array) App::get_by_where('pk_customer_leads',array( 'leads_id' => $postdata["leads_id"]  ));
    }
    ...
    function assignleads(){
        if( isset($_POST['assignlead'])){
            ...
            $this->db->where("id NOT IN (SELECT customer.leads_id
                               FROM customer)");
            $this->db->where('first_name',$_POST['assignlead']);
            $data['leads_name'] = $this->db->get()->result();
