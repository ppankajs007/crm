...
$globalMethods = array('index','edit', 'add', 'emails','product','quote_orders','wcOrders');
?>
...
<script>
	...
	var assemble_val = jQuery('[name=assemble_value]:checked').val();
		jQuery.ajax({
			url:"<?php echo base_url('orders/wcOrders/productByCategory') ?>",
			type: "post",
...
<script>    
 jQuery( document ).on('change','#vendorName',function(){
		...
		if ( v_id == 5 ) {
			jQuery.ajax({
				url:"<?php echo base_url('orders/wcOrders/styleByVendor') ?>",
				type: "post",
				data:{ v_id: v_id },
				success:function(res){
    ...
    jQuery(document).on('click','.catDrop',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/wcOrders/categoryByStyle'); ?>",
			type:"post",
			data:{ style_id:sID},
			success:function(res){
    ...
    jQuery(document).on('click','#mainCats li',function(){
        ...
        jQuery('.resList ul').hide();
        jQuery.ajax({
			url:"<?php echo base_url('orders/wcOrders/categoryByProduct'); ?>",
			type:"post",
    ...
    jQuery(document).on('keyup input', '#searchPro', function (e) {
        ...
        if( srch.length > 1 ){
		    ...
		    jQuery('.resList ul#proRes').show();
            jQuery.ajax({
    			url:"<?php echo base_url('orders/wcOrders/productFilter'); ?>",
    			type:"post",
    ...
    function updateQty(uniquePro){
		...
		jQuery.ajax({
			async:false,
			url:"<?php echo base_url('orders/wcOrders/calcuateProQty'); ?>",
			type:"post",
 	...
 	// jQuery(document).ready(function(){
	 //    jQuery('.editProTable tbody tr').each(function(){
		// 	jQuery(this).find('.qty').trigger("change");
		// 	console.log(jQuery(this).find('.qty').val());
	 //    })
 	// })
    ...
    function calcuateAllProductPrice( updateDiscountAmt = true ){
		...
		var paid_amount  = jQuery( '#paidAmount' ).val();
 		...
 		jQuery.ajax({
			url:"<?php echo base_url('orders/wcOrders/calcuateAllProductPrice'); ?>",
			type:"post",
			data:{ 
	...
	jQuery( document ).on('keyup','.price_perc',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/wcOrders/calcuateAllProductByPerc'); ?>",
			type:"post",
			data:{ 
				total_price:total_sale_price, 
				perc:perc, 
	...
	jQuery( document ).on('keyup','.perc_amount',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/wcOrders/calcuateAllProductByAmt'); ?>",
			type:"post",
			data:{ 
				total_price : total_sale_price, 
				disc_amt    : disc_amt, 
	...
	jQuery( document ).on('keyup','.net_price',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/wcOrders/calcuateAllProductByNetPrice'); ?>",
			type:"post",
	...
	jQuery( document ).on('change','.resale_certificate',function(){
		var resale = taxCostActive();
		calcuateAllProductPrice(); return false;
		calcuateTax(resale);
	...
	function calcuateTax(resale){
		...
		var perc_amount = jQuery( '.perc_amount' ).val();
		jQuery.ajax({
			url: '<?php echo base_url('orders/wcOrders/calcuateTax'); ?>',
			type: 'post',
	...
	jQuery(document).on('keyup','.delivery_price_inp', function(){
		...
		var subtotal 	 = jQuery('.subtotal' ).val();
		jQuery.ajax({
			url: '<?php echo base_url('orders/wcOrders/customSum'); ?>',
			type: 'post',
	...
	function calculateDiscount(){
		...
		var delivery_price  = jQuery( '.delivery_price_inp' ).val();
		var resale       	= taxCostActive();
		console.log( resale + " here" );
    	jQuery.ajax({
			url:"<?php echo base_url('orders/wcOrders/calcuateNetDiscount'); ?>",
			type:"post",
			data:{ 
				price_perc 	    : price_perc, 
	...
	jQuery( document ).on('click','.add_child',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/wcOrders/childProductStyle'); ?>",
			type:'post',
	...
	jQuery( document ).on('click','.add_subchild',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/wcOrders/subChildProductStyle'); ?>",
			type:'post',
			data:{ 
					item_code:item_code, 
					style_id:style_id, 
	...
	function getSubChildProduct(productId,data_row_count,assemble_value){
		jQuery.ajax({
			url:"<?php echo base_url('orders/wcOrders/productByStyleName'); ?>",
			type:'post',
	...
	jQuery( document ).on('click','#importcustpro', function(e){
	    ...
	    setTimeout(function(){ 
			jQuery.ajax({
				url: "<?php echo base_url('orders/wcOrders/edit/'); ?>"+orderID,
			    type: "post",
			    data: formData,
			    contentType: false,
			    processData: false,
	...
	jQuery( document ).on('click','#save_custom_product',function(){
	    ...
	    jQuery( 'body' ).append( "<div class='loaderOverlay'><div class='inner'><img src='https://cdn-camp.mini-sites.net/Publish/53c7f5697dd94d829fcb41e20510e344/9acad7d360bd4f9a8b85ff13b1c61aac/src/images/loader01.gif' /></div></div>" );
		jQuery.ajax({
			url: "<?php echo base_url('orders/wcOrders/addCustomProduct'); ?>",
		    type: "post",
		    data: formData,
		    contentType: false,
		    processData: false,
