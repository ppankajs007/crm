<?php 
	...
	$globalMethods = array('index','edit', 'add', 'emails','product','quote_orders','TsgOrders');
...
<script>
	...
	function add_product(productId) {
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/TsgOrders/productByCategory') ?>",
			type: "post",
...
<script>    
    jQuery( document ).on('change','#vendorName',function(){
		...
		if ( v_id == 1 ) {
			jQuery.ajax({
				url:"<?php echo base_url('orders/TsgOrders/styleByVendor') ?>",
				type: "post",
				data:{ v_id: v_id },
				success:function(res){
    ...
    jQuery(document).on('click','.catDrop',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/TsgOrders/categoryByStyle'); ?>",
			type:"post",
			data:{ style_id:sID},
			success:function(res){
    ...
    jQuery(document).on('click','#mainCats li',function(){
        ...
        jQuery('.resList ul').hide();
        jQuery.ajax({
			url:"<?php echo base_url('orders/TsgOrders/categoryByProduct'); ?>",
			type:"post",
    ...
    jQuery(document).on('click','#childCats li',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/TsgOrders/categoryByProduct'); ?>",
			type:"post",
			data:{ cat_id : cat_id, s_name:s_name, main_cat : main_cat, cat_type : 'sub_category_first'},
			success:function(res){
    ...
    jQuery(document).on('click','#subChild li',function(){
        ...
        jQuery.ajax({
			url:"<?php echo base_url('orders/TsgOrders/categoryByProduct'); ?>",
			type:"post",
			data:{ cat_id : cat_id, s_name:s_name, main_cat : main_cat, child_cat : child_cat, cat_type : 'sub_category_second'},
			success:function(res){
			    var resp = jQuery.parseJSON(res);
			    if(resp.status == 'notfinal'){
    ...
    jQuery(document).on('click','.childCats .catHolder',function(){
        ...
        jQuery.ajax({
			url:"<?php echo base_url('orders/TsgOrders/categoryByProduct'); ?>",
			type:"post",
			data:{ cat_id : cat_id, s_name:s_name, main_cat : main_cat, child_cat : child_cat, cat_type : 'sub_category_first'},
			success:function(res){
			    var resp = jQuery.parseJSON(res);
			    if(resp.status == 'notfinal'){
    			    jQuery('.resList ul#subChild').html(resp.res);
    ...
    jQuery(document).on('click','.subChild .catHolder',function(){
        ...
        jQuery.ajax({
			url:"<?php echo base_url('orders/TsgOrders/categoryByProduct'); ?>",
			type:"post",
			data:{ cat_id : cat_id, s_name:s_name, main_cat : main_cat, child_cat : child_cat, sub_cat : sub_cat, cat_type : 'sub_category_second'},
			success:function(res){
			    var resp = jQuery.parseJSON(res);
			    if(resp.status == 'notfinal'){
    			    jQuery('.resList ul#subChild').html(resp.res);
    ...
    jQuery(document).on('click','.subSubChild .catHolder',function(){
        ...
        jQuery.ajax({
			url:"<?php echo base_url('orders/TsgOrders/categoryByProduct'); ?>",
			type:"post",
			data:{ cat_id : cat_id, s_name:s_name, main_cat : main_cat, child_cat : child_cat, sub_cat : sub_cat, sub_sub_cat : sub_sub_cat, cat_type : 'sub_category_second'},
			success:function(res){
			    var resp = jQuery.parseJSON(res);
			    jQuery('.resList ul#proRes').html(resp.res);
			    jQuery('.resList ul#proRes').show();
    ...
    jQuery(document).on('keyup input', '#searchPro', function (e) {
        ...
        if( srch.length > 1 ){
		    ...
		    jQuery('.resList ul#proRes').show();
            jQuery.ajax({
    			url:"<?php echo base_url('orders/TsgOrders/productFilter'); ?>",
    			type:"post",
    ...
    function updateQty(uniquePro){
		...
		jQuery.ajax({
			async:false,
			url:"<?php echo base_url('orders/TsgOrders/calcuateProQty'); ?>",
			type:"post",
    ...
    function calcuateAllProductPrice( updateDiscountAmt = true ){
		...
		var paid_amount  = jQuery( '#paidAmount' ).val();
 		...
 		jQuery.ajax({
			url:"<?php echo base_url('orders/TsgOrders/calcuateAllProductPrice'); ?>",
			type:"post",
			data:{ 
	...
	jQuery( document ).on('keyup','.perc_amount',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/TsgOrders/calcuateAllProductByAmt'); ?>",
			type:"post",
			data:{ 
				total_price : total_sale_price, 
				disc_amt    : disc_amt, 
	...
	jQuery( document ).on('keyup','.price_perc',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/TsgOrders/calcuateAllProductByPerc'); ?>",
			type:"post",
			data:{ 
				total_price:total_sale_price, 
				perc:perc, 
	...
	jQuery( document ).on('keyup','.net_price',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/TsgOrders/calcuateAllProductByNetPrice'); ?>",
			type:"post",
	...
	function calcuateTax(resale){
		...
		var resale = taxCostActive();
		jQuery.ajax({
			url: '<?php echo base_url('orders/TsgOrders/calcuateTax'); ?>',
			type: 'post',
	...
	jQuery(document).on('keyup','.delivery_price_inp', function(){
		...
		var subtotal 	 = jQuery('.subtotal' ).val();
		jQuery.ajax({
			url: '<?php echo base_url('orders/TsgOrders/customSum'); ?>',
			type: 'post',
	...
	jQuery( document ).on('click','.add_child',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/TsgOrders/childProductStyle'); ?>",
			type:'post',
	...
	jQuery( document ).on('click','.add_subchild',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/TsgOrders/subChildProductStyle'); ?>",
			type:'post',
			data:{ 
					item_code:item_code, 
					style_id:style_id, 
					data_row_count:data_row_count,
					data_parent_row:data_parent_row, 
	...
	function getSubChildProduct(productId,data_row_count,assemble_value){
		jQuery.ajax({
			url:"<?php echo base_url('orders/TsgOrders/productByStyleName'); ?>",
			type:'post',
			...
			success:function(res){
					...
					jQuery('#add_subchild'+data_row_count).attr('data_style_id',ress.style_id);
					jQuery('#add_subchild'+data_row_count).attr('data_item_code',ress.Item_Code);
	...
	jQuery( document ).on('click','#importcustpro', function(e){
	    ...
	    setTimeout(function(){ 
			jQuery.ajax({
				url: "<?php echo base_url('orders/TsgOrders/edit/'); ?>"+orderID,
			    type: "post",
			    data: formData,
			    contentType: false,
			    processData: false,
	...
	jQuery( document ).on('click','#save_custom_product',function(){
	    ...
	    jQuery( 'body' ).append( "<div class='loaderOverlay'><div class='inner'><img src='https://cdn-camp.mini-sites.net/Publish/53c7f5697dd94d829fcb41e20510e344/9acad7d360bd4f9a8b85ff13b1c61aac/src/images/loader01.gif' /></div></div>" );
		jQuery.ajax({
			url: "<?php echo base_url('orders/TsgOrders/addCustomProduct'); ?>",
		    type: "post",
		    data: formData,
		    contentType: false,
		    processData: false,
