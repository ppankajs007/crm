...
$globalMethods = array('index','edit', 'add', 'emails','product','quote_orders','cncOrders');
?>
...
<script>
	...
	function add_product(productId) {
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/cncOrders/productByCategory') ?>",
			type: "post",
...
<script>    
    jQuery( document ).on('change','#vendorName',function(){
		...
		if ( v_id == 3 ) {
			jQuery.ajax({
				url:"<?php echo base_url('orders/cncOrders/styleByVendor') ?>",
				type: "post",
				data:{ v_id: v_id },
				success:function(res){
		...
		if ( v_id == 4 ) {
			...
			var msi_url = "<?php echo base_url(); ?>orders/AsgfOrders/edit/"+o_id;
		}else if ( v_id == 9 ) {
			var msi_url = "<?php echo base_url(); ?>orders/centuryOrders/edit/"+o_id;
			window.location.href = msi_url;
		}else{
    ...
    jQuery(document).on('click','.catDrop',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/cncOrders/categoryByStyle'); ?>",
			type:"post",
			data:{ style_id:sID},
			success:function(res){
    ...
    jQuery(document).on('click','#mainCats li',function(){
        ...
        jQuery('.resList ul').hide();
        jQuery.ajax({
			url:"<?php echo base_url('orders/cncOrders/categoryByProduct'); ?>",
			type:"post",
    ...
    jQuery(document).on('keyup input', '#searchPro', function (e) {
        ...
        if( srch.length > 1 ){
		    ...
		    jQuery('.resList ul#proRes').show();
            jQuery.ajax({
    			url:"<?php echo base_url('orders/cncOrders/productFilter'); ?>",
    			type:"post",
    ...
    function updateQty(uniquePro){
		...
		jQuery.ajax({
			async:false,
			url:"<?php echo base_url('orders/cncOrders/calcuateProQty'); ?>",
			type:"post",
    ...
    function calcuateAllProductPrice( updateDiscountAmt = true ){
		...
		var paid_amount  = jQuery( '#paidAmount' ).val();
 		...
 		jQuery.ajax({
			url:"<?php echo base_url('orders/cncOrders/calcuateAllProductPrice'); ?>",
			type:"post",
			data:{ 
	...
	jQuery( document ).on('keyup','.price_perc',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/cncOrders/calcuateAllProductByPerc'); ?>",
			type:"post",
			data:{ 
				total_price:total_sale_price, 
				perc:perc, 
	...
	jQuery( document ).on('keyup','.perc_amount',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/cncOrders/calcuateAllProductByAmt'); ?>",
			type:"post",
			data:{ 
				total_price : total_sale_price, 
				disc_amt    : disc_amt, 
	...
	jQuery( document ).on('keyup','.net_price',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/cncOrders/calcuateAllProductByNetPrice'); ?>",
			type:"post",
	...
	function calcuateTax(resale){
		...
		var resale = taxCostActive();
		jQuery.ajax({
			url: '<?php echo base_url('orders/cncOrders/calcuateTax'); ?>',
			type: 'post',
	...
	jQuery(document).on('keyup','.delivery_price_inp', function(){
		...
		var subtotal 	 = jQuery('.subtotal' ).val();
		jQuery.ajax({
			url: '<?php echo base_url('orders/cncOrders/customSum'); ?>',
			type: 'post',
	...
	jQuery( document ).on('click','.add_child',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/cncOrders/childProductStyle'); ?>",
			type:'post',
	...
	jQuery( document ).on('click','.add_subchild',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/cncOrders/subChildProductStyle'); ?>",
			type:'post',
	...
	function getSubChildProduct(productId,data_row_count,assemble_value){
		jQuery.ajax({
			url:"<?php echo base_url('orders/cncOrders/productByStyleName'); ?>",
			type:'post',
	...
	jQuery( document ).on('click','#importcustpro', function(e){
	    ...
	    setTimeout(function(){ 
			jQuery.ajax({
				url: "<?php echo base_url('orders/cncOrders/edit/'); ?>"+orderID,
			    type: "post",
			    data: formData,
			    contentType: false,
			    processData: false,
	...
	jQuery( document ).on('click','#save_custom_product',function(){
	    ...
	    jQuery( 'body' ).append( "<div class='loaderOverlay'><div class='inner'><img src='https://cdn-camp.mini-sites.net/Publish/53c7f5697dd94d829fcb41e20510e344/9acad7d360bd4f9a8b85ff13b1c61aac/src/images/loader01.gif' /></div></div>" );
		jQuery.ajax({
			url: "<?php echo base_url('orders/cncOrders/addCustomProduct'); ?>",
		    type: "post",
		    data: formData,
		    contentType: false,
		    processData: false,
