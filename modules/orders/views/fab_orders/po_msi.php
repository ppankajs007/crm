         ...
         <div class="col-12">
            <div class="card-box col-md-12">
                            ...
                            <?php
                              if ( !empty($fab_products) ) {
                                  ...
                                  foreach ($fab_products  as $key => $value){
                                      ...
                                      if ( !empty($value['slab_one_id']) || !empty($value['slab_one_sqft']) ) {
                                          $total_price = $value['slab_one_sqft'] * $value['item_cost'];
                                          $total_order[] = $total_price; 
                                          $value['total_price'] = $total_price;
                                          $value['slab_id']     = $value['slab_one_id'];
                                          $value['slab_sqft']   = $value['slab_one_sqft'];
                                          $this->view( 'fab_orders/tr__template_pomsi',$value);
                                      }

                                      if ( !empty($value['slab_two_id']) || !empty($value['slab_two_sqft']) ) {
                                            $total_price = $value['slab_two_sqft'] * $value['item_cost'];
                                            $total_order[] = $total_price; 
                                            $value['total_price'] = $total_price;
                                      ...
                                      if ( !empty($value['slab_three_id']) || !empty($value['slab_three_sqft']) ) {
                                            $total_price = $value['slab_three_sqft'] * $value['item_cost'];
                                            $total_order[] = $total_price; 
                                            $value['total_price'] = $total_price;
               ...
               <div class="row">
                  ...
                  <br><br><br>
                  <?php if($edit_order['payment_person'] == 'no'){ ?>
                  <div class="pPerson">
                     <h5>
                        I, <?php echo $edit_order['payment_person_name']; ?> authorize the use of my credit card to pay for this order. 
                     </h5>
                     <br>
