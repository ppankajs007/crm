<?php 
	...
	$globalMethods = array('index','edit', 'add', 'emails','product','quote_orders','msiTileOrders');
...
<script>
	...
	var assemble_val = jQuery('[name=assemble_value]:checked').val();
		jQuery.ajax({
			url:"<?php echo base_url('orders/msiTileOrders/productByCategory') ?>",
			type: "post",
...
<script>    
    jQuery( document ).on('change','#vendorName',function(){
		...
		if ( v_id == 7 ) {
			jQuery.ajax({
				url:"<?php echo base_url('orders/msiTileOrders/styleByVendor') ?>",
				type: "post",
				data:{ v_id: v_id },
				success:function(res){
    ...
    jQuery(document).on('click','.catDrop',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/msiTileOrders/categoryByStyle'); ?>",
			type:"post",
			data:{ style_id:sID},
			success:function(res){
    ...
    jQuery(document).on('click','#mainCats li',function(){
        ...
        jQuery('.resList ul').hide();
        jQuery.ajax({
			url:"<?php echo base_url('orders/msiTileOrders/categoryByProduct'); ?>",
			type:"post",
			data:{ cat_id : cat_id, s_name:s_name, cat_type : 'sub_1_category'},
			success:function(res){
    ...
    jQuery(document).on('click','#childCats li',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/msiTileOrders/categoryByProduct'); ?>",
			type:"post",
			data:{ cat_id : cat_id, s_name:s_name, main_cat : main_cat, cat_type : 'sub_2_category'},
			success:function(res){
  ...
  //   });
    ...
    jQuery(document).on('click','.subChild .catHolder',function(){
        ...
        jQuery.ajax({
			url:"<?php echo base_url('orders/msiTileOrders/categoryByProduct'); ?>",
			type:"post",
			data:{ cat_id : cat_id, s_name:s_name, main_cat : main_cat, child_cat : child_cat, sub_cat : sub_cat, cat_type : 'sub_3_category'},
			success:function(res){
			    var resp = jQuery.parseJSON(res);
			    if(resp.status == 'notfinal'){
    			    jQuery('.resList ul#subChild').html(resp.res);
  ...
  //   });
    ...
    jQuery(document).on('keyup input', '#searchPro', function (e) {
        ...
        if( srch.length > 1 ){
		    ...
		    jQuery('.resList ul#proRes').show();
            jQuery.ajax({
    			url:"<?php echo base_url('orders/msiTileOrders/productFilter'); ?>",
    			type:"post",
	...
	jQuery( document ).on('keyup','.change_cost , .change_price',function(){

		var data_row_count = jQuery(this).attr('data-proinput');
		var input_value    = jQuery('#sqbox_input'+data_row_count).val();
		var data_ctype     = jQuery('#sqbox_input'+data_row_count).attr('data_ctype');
		var proId 		   = jQuery('#proId'+data_row_count).val();
		var change_price   = jQuery('#change_price'+data_row_count).val();
		var change_cost   = jQuery('#change_cost'+data_row_count).val();

		jQuery.ajax({
			url:  '<?php echo base_url('orders/msiTileOrders/calcuateProductType') ?>',
			type: 'post',
			data: { input_value:input_value,data_ctype:data_ctype,proId:proId,change_price:change_price,change_cost:change_cost  },
			success:function(res){
				var dbRes = jQuery.parseJSON(res);
				console.log(dbRes);
				// jQuery('#qty'+data_row_count).removeAttr('readonly');
				jQuery('#qty'+data_row_count).val(dbRes.box_to_order);
				// jQuery('#qty'+data_row_count).attr('readonly','true');
				jQuery('#cost_price'+data_row_count).attr('data-itemCost',dbRes.item_cost);
				jQuery('#cost_price'+data_row_count).html('<span class="morkpamt">$'+dbRes.item_cost+'</span> (<span class="morkpperc">'+dbRes.calper+'</span>%)');
				jQuery('#final_cost_input'+data_row_count).val(dbRes.item_cost);
				jQuery('#sale_price'+data_row_count).text('$'+dbRes.item_price);
				jQuery('#sale_price_input'+data_row_count).val(dbRes.item_price);
				calcuateAllProductPrice();
			}
		})
	});
    ...
    function updateQty(uniquePro){
    	...
    	var data_ctype     = jQuery('#sqbox_input'+uniquePro).attr('data_ctype');
    	var input_value    = jQuery('#sqbox_input'+uniquePro).val();
		jQuery.ajax({
			async:false,
			url:"<?php echo base_url('orders/msiTileOrders/calcuateProQty'); ?>",
			type:"post",
			data:{ qty:totalQty, pro_id:pro_id, change_cost:change_cost, change_price:change_price,data_ctype:data_ctype,input_value:input_value},
			success:function(res){
				console.log(res);
	...
	function calcuateTax(resale){
		...
		jQuery.ajax({
			...
			type: 'post',
			data:{ subtotal,perc_amount,taxVal },
			success:function(res){
					var calPrice = jQuery.parseJSON(res);
	...
	jQuery( document ).on('change','.has_a_uez',function(){
		...
		var has_a_stform = jQuery(this).val();
		if (has_a_stform != 'yes') {
			jQuery('.has_a_stform_cer').addClass('dspln');
		}else{
			jQuery('.has_a_stform_cer').removeClass('dspln');
		}
		calcuateAllProductPrice();
	});
	...
	jQuery( document ).on('click','.add_child',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/msiTileOrders/childProductStyle'); ?>",
			type:'post',
			data:{ item_code:item_code,style_id:style_id,parent_id:parent_id,data_row_count:data_row_count },
			success:function(res){
				console.log(res);
				jQuery('#pro_'+data_parent_id).after(res);
				jQuery('.removetr').remove();
			}
		});
	...
	function getSubChildProduct(productId,data_row_count,assemble_value){
		jQuery.ajax({
			url:"<?php echo base_url('orders/msiTileOrders/productByStyleName'); ?>",
			type:'post',
			data:{ productId:productId,assemble_value:assemble_value },
			success:function(res){
				var ress = jQuery.parseJSON(res);
				jQuery('#description'+data_row_count).html(ress.style_name+'/'+ress.item_code);
				...
				jQuery('#add_child'+data_row_count).attr('data_style_id',ress.styid);
	...
	jQuery(document).on('keyup','.sqbox_input',function(){
		var data_row_count = jQuery(this).attr('data_row_count');
		var input_value    = jQuery(this).val();
		var data_ctype     = jQuery(this).attr('data_ctype');
		var proId 		   = jQuery('#proId'+data_row_count).val();
		var change_price   = jQuery('#change_price'+data_row_count).val();
		var change_cost   = jQuery('#change_cost'+data_row_count).val();
		jQuery.ajax({
	...
	jQuery( document ).on('click','#importcustpro', function(e){
		...
		jQuery.ajax({
			url: "<?php echo base_url('orders/msiTileOrders/edit/'); ?>"+orderID,
		    type: "post",
		    data: formData,
		    contentType: false,
		    processData: false,
