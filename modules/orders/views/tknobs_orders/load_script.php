...
$globalMethods = array('index','edit', 'add', 'emails','product','quote_orders','tknobsOrders');
?>
...
<script>
	...
	function add_product(productId) {
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/tknobsOrders/productByCategory') ?>",
			type: "post",
...
<script>    
    jQuery( document ).on('change','#vendorName',function(){
		...
		if ( v_id == 6 ) {
			jQuery.ajax({
				url:"<?php echo base_url('orders/tknobsOrders/styleByVendor') ?>",
				type: "post",
				data:{ v_id: v_id },
				success:function(res){
    ...
    jQuery(document).on('click','.catDrop',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/tknobsOrders/categoryByStyle'); ?>",
			type:"post",
			data:{ style_id:sID},
			success:function(res){
    ...
    jQuery(document).on('click','#mainCats li',function(){
        ...
        jQuery('.resList ul').hide();
        jQuery.ajax({
			url:"<?php echo base_url('orders/tknobsOrders/categoryByProduct'); ?>",
			type:"post",
    ...
    jQuery(document).on('keyup input', '#searchPro', function (e) {
        ...
        if( srch.length > 1 ){
		    ...
		    jQuery('.resList ul#proRes').show();
            jQuery.ajax({
    			url:"<?php echo base_url('orders/tknobsOrders/productFilter'); ?>",
    			type:"post",
    ...
    function updateQty(uniquePro){
		...
		jQuery.ajax({
			async:false,
			url:"<?php echo base_url('orders/tknobsOrders/calcuateProQty'); ?>",
			type:"post",
    ...
    function calcuateAllProductPrice( updateDiscountAmt = true ){
		...
		var paid_amount  = jQuery( '#paidAmount' ).val();
 		...
 		jQuery.ajax({
			url:"<?php echo base_url('orders/tknobsOrders/calcuateAllProductPrice'); ?>",
			type:"post",
			data:{ 
	...
	jQuery( document ).on('keyup','.perc_amount',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/tknobsOrders/calcuateAllProductByAmt'); ?>",
			type:"post",
			data:{ 
				total_price : total_sale_price, 
				disc_amt    : disc_amt, 
	...
	jQuery( document ).on('keyup','.price_perc',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/tknobsOrders/calcuateAllProductByPerc'); ?>",
			type:"post",
			data:{ 
				total_price:total_sale_price, 
				perc:perc, 
	...
	jQuery( document ).on('keyup','.net_price',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/tknobsOrders/calcuateAllProductByNetPrice'); ?>",
			type:"post",
	...
	jQuery(document).on('keyup','.delivery_price_inp', function(){
		...
		jQuery.ajax({
			url: '<?php echo base_url('orders/tknobsOrders/customSum'); ?>',
			type: 'post',
	...
	jQuery( document ).on('click','.add_child',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/tknobsOrders/childProductStyle'); ?>",
			type:'post',
	...
	function getSubChildProduct(productId,data_row_count,assemble_value){
		jQuery.ajax({
			url:"<?php echo base_url('orders/tknobsOrders/productByStyleName'); ?>",
			type:'post',
	...
	jQuery( document ).on('click','.add_subchild',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/tknobsOrders/subChildProductStyle'); ?>",
			type:'post',
			data:{ 
					item_code:item_code, 
					style_id:style_id, 
	...
	jQuery( document ).on('click','#importcustpro', function(e){
		...
		jQuery.ajax({
			url: "<?php echo base_url('orders/tknobsOrders/edit/'); ?>"+orderID,
		    type: "post",
		    data: formData,
		    contentType: false,
		    processData: false,
