<?php 
	...
	$globalMethods = array('index','edit', 'add', 'emails','product','quote_orders','AsfgOrders');
?>
...
<script>
	...
	var assemble_val = jQuery('[name=assemble_value]:checked').val();
		jQuery.ajax({
			url:"<?php echo base_url('orders/AsfgOrders/productByCategory') ?>",
			type: "post",
...
<script>    
     jQuery( document ).on('change','#vendorName',function(){
		...
		if ( v_id == 8 ) {
			jQuery.ajax({
				url:"<?php echo base_url('orders/AsfgOrders/styleByVendor') ?>",
				type: "post",
				data:{ v_id: v_id },
				success:function(res){
    ...
    jQuery(document).on('click','.catDrop',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/AsfgOrders/categoryByStyle'); ?>",
			type:"post",
			data:{ style_id:sID},
			success:function(res){
    ...
    jQuery(document).on('click','#mainCats li',function(){
        ...
        jQuery('.resList ul').hide();
        jQuery.ajax({
			url:"<?php echo base_url('orders/AsfgOrders/categoryByProduct'); ?>",
			type:"post",
    ...
    jQuery(document).on('keyup input', '#searchPro', function (e) {
        ...
        if( srch.length > 1 ){
		    ...
		    jQuery('.resList ul#proRes').show();
            jQuery.ajax({
    			url:"<?php echo base_url('orders/AsfgOrders/productFilter'); ?>",
    			type:"post",
    ...
    function updateQty(uniquePro){
		...
		jQuery.ajax({
			async:false,
			url:"<?php echo base_url('orders/AsfgOrders/calcuateProQty'); ?>",
			type:"post",
    ...
    function calcuateAllProductPrice(updateDiscountAmt = true){
		...
		var paid_amount  = jQuery( '#paidAmount' ).val();
 		...
 		jQuery.ajax({
			url:"<?php echo base_url('orders/asfgOrders/calcuateAllProductPrice'); ?>",
			type:"post",
			data:{ 
	...
	jQuery(document).on('keyup','.delivery_price_inp', function(){
		...
		jQuery.ajax({
			url: '<?php echo base_url('orders/asfgOrders/customSum'); ?>',
			type: 'post',
	...
	jQuery(document).on('keyup','.delivery_cost_inp', function(){
	  ...
	  jQuery( document ).on('keyup','.perc_amount',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/asfgOrders/calcuateAllProductByAmt'); ?>",
			type:"post",
			data:{ 
				total_price : total_sale_price, 
				disc_amt    : disc_amt, 
	...
	jQuery( document ).on('keyup','.price_perc',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/asfgOrders/calcuateAllProductByPerc'); ?>",
			type:"post",
			data:{ 
				total_price:total_sale_price, 
				perc:perc, 
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/asfgOrders/calcuateNetDiscount'); ?>",
			type:"post",
			data:{ 
				price_perc 	    : price_perc, 
	...
	function calcuateTax(resale){
		...
		var perc_amount = jQuery( '.perc_amount' ).val();
		jQuery.ajax({
			url: '<?php echo base_url('orders/asfgOrders/calcuateTax'); ?>',
			type: 'post',
	...
	jQuery( document ).on('keyup','.net_price',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/asfgOrders/calcuateAllProductByNetPrice'); ?>",
			type:"post",
	...
	jQuery( document ).on('click','.add_subchild',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/AsfgOrders/subChildProductStyle'); ?>",
			type:'post',
			...
			success:function(res){
				console.log(res);
				jQuery('#pro_'+parent_id).after(res);
				jQuery('.removetr').remove();
			}
		});
	...
	function getSubChildProduct(productId,data_row_count,assemble_value){
		jQuery.ajax({
			url:"<?php echo base_url('orders/AsfgOrders/productByStyleName'); ?>",
			type:'post',
	...
	jQuery( document ).on('click','#importcustpro', function(e){
		...
		jQuery.ajax({
			url: "<?php echo base_url('orders/AsfgOrders/edit/'); ?>"+orderID,
		    type: "post",
		    data: formData,
		    contentType: false,
		    processData: false,
