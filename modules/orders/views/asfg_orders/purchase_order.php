                                  ...
                                  $rawTotal = array();
                                    $total_sum = 0;
                                      foreach ($pro_view as $key => $value) {
                                            $parentProduct['data'] = $controllerThis->quoteOrderForObject($order_view['assemble_value'],$value,'Yes');
                                            $rawTotal[] = $parentProduct['data']['totalprice'];
                                            $this->view( 'asfg_orders/tr__template_quote',$parentProduct);

                                            if ( !empty($product_order_child) ) {
                                              foreach ($product_order_child as $key => $values) {
                                                  if( $value->POProId == $values->pro_order_id  ){ 

                                                      $childProduct['data'] = $controllerThis->quoteOrderForObject($order_view['assemble_value'],$values,'Yes');

                                                      $childProduct['data']['child'] = 1;

                                                      $rawTotal[] = $childProduct['data']['totalprice'];

                                                      $this->view( 'asfg_orders/tr__template_quote',$childProduct);
                                                      ...
                                                      if ( !empty($product_order_sub_child) ) {
                                                        foreach ($product_order_sub_child as $key => $valuesub) {
                                                            if( $valuesub->pro_parent_id == $values->product_id  ){
                                                               ...
                                                               $subchildProduct['data'] = $controllerThis->quoteOrderForObject($order_view['assemble_value'],$valuesub,'Yes');
                                                                $subchildProduct['data']['subchild'] = 1;
                                                      
                                                                $rawTotal[] = $subchildProduct['data']['totalprice'];

                                                                $this->view( 'asfg_orders/tr__template_quote',$subchildProduct);
                                                            }
                                                        }
                                                      }
                                                  }
               ...
               <div class="row">
                  ...
                  <div class="col-sm-6">
                     <div class="float-right">
                        <?php $total_sum = $total_sum + $order_view['delivery_cost']; ?>
                        <p><b>Total : </b><span class="float-right"> &nbsp;&nbsp;&nbsp; 
                           $<?= decimalValue($total_sum); ?></span>
                        </p>
                     </div>
                     </div>
