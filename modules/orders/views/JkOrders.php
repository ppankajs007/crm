...
class JkOrders extends MX_Controller
{
    ...
    function edit($id){
                        ...
                        foreach ($sheet->getRowIterator() as $row) { 
                            if ($count > 1) {
                                ...
                                $this->db->select('pkpro.*,pkpro.id as pkpro_id,pkpro.style_id as pkprost,sy.*,sy.id as sid');
                                         $this->db->from('jk_product pkpro');
                                         $this->db->join('jk_style sy', 'pkpro.style_id = sy.id', 'left');
                                ...
                                if ( $pro_Id != "" ) {
                                      ...
                                      if( $edit_order['assemble_value'] == 1 ){
                                        $item_cost    = $pro_Id->item_cost_unassembled_with_tariff;
                                        $cabinet_cost = $pro_Id->unassembled_item_cost;
                                      } else{
                                        $item_cost    = $pro_Id->item_assembly_cost;
                                        $cabinet_cost = $pro_Id->assembled_item_cost;
                                      }
                                      ...
                                      $proDode = array( 'product_id' => $pro_Id->pkpro_id, 'qty' => $qty, 'description' => '','descriptionII' => '','item_code' => $itmCode,'style_id' => $styid,'price'=> '','u_price'=> '','total_price' => $cabinet_cost,'final_cost' => $item_cost,'item_cost' => $item_cost,'cabinaet_assemb_c' => $cabinet_cost ,'is_parent' => $is_parent, 'parent_id' => $setParent);
                                        ...
                                        $varn_Id = $this->jk_model->getProductFindByVariants($itmCode,$styid)[0];
                                        ...
                                        if ( !empty($varn_Id) ) {
                                            ...
                                            if( $edit_order['assemble_value'] == 1 ){
                                                $item_cost    = $varn_Id['item_cost_unassembled_with_tariff'];
                                                $cabinet_cost = $varn_Id['unassembled_item_cost'];
                                            } else{
                                                $item_cost    = $varn_Id['item_assembly_cost'];
                                                $cabinet_cost = $varn_Id['assembled_item_cost'];
                                            }
                                            ...
                                            $not_exist_product = [ 'item_code' => $itmCode, 'style_id' => $styid , 'vendor_id' => $this->input->post('vendor') , 'not_exist' => 'Yes' ];

                                            $not_exist_pro_id = $this->jk_model->saveNotExistProduct($not_exist_product);

                                            $proDode = array( 'product_id' => $not_exist_pro_id, 'qty' => '0', 'item_code' => $itmCode,'style_id' => $styid,'description' => '','descriptionII' => '','price'=> '','u_price'=> '','item_cost' => 0,'cabinaet_assemb_c' => 0,'total_price' => '','final_cost' => '', 'is_parent' => $is_parent, 'parent_id' => $setParent,'' => 0, 'not_exist' => 'Yes' );
        ...
        //         }
            ...
            $this->jk_model->saveJkOrder( $id, $sArr );
            log_orders( 
                $id, //order_id 
        ...
        $data['edit_order']            = $this->jk_model->getJkOrder($id);
        $data['vendor']                = $this->jk_model->fetch_all('vendor');
        $data['jk_style']              = $this->jk_model->fetch_all('jk_style');
        $data['jk_products']           = $this->jk_model->getJkProdyctByOrderId($id);
        $data['jk_products_child']     = $this->jk_model->getJkProdyctChildByOrderId($id);
        $data['jk_products_subchild']  = $this->jk_model->getJkProdyctSubChildByOrderId($id);
        $data['jkClass']               = $this;

        $this->template->title('Edit');
        $this->template
                ->set_layout('inner')
                ->build('jk_orders/jk_edit', $data);
    }
    ...
    public function categoryByStyle() {
        ...
        $allCats = $this->jk_model->getCatByStyle($styleID);
        $options = "";
    ...
    public function categoryByProduct(){
        ...
        $rawRes = $this->jk_model->getCatByProduct($cat_id,$cat_type,$p_style,$res_status,$mainCat,$childCat,$subCat, $subSubCat);
        $options = "";
        $dbRes = $rawRes['result'];
    ...
    function productByCategory()
    {
        ...
        if ( isset( $_POST['productId'] ) ) {
            ...
            $dbRes     = $this->jk_model->getProductByCat($productId);
            if ($dbRes['status'] == 'success') {
                foreach ($dbRes['rows'] as $key => $value) {

                    if ( $_POST['assemble_val'] == '1' ) {
                            $item_cost = $value['item_cost_unassembled_with_tariff'];
                            $item_price = $value['unassembled_item_cost'];
                    }else{
                        $item_cost  = $value['item_assembly_cost'];
                        $item_price = $value['assembled_item_cost'];
                    }

                    $value['item_cost']  = decimalValue($item_cost);
                    $value['item_code']  = $value['item_code'];
                    $value['item_description'] = $value['item_description'];
                    $value['item_price'] = decimalValue($item_price);   
                    $value['prcnt_cost'] = $this->calcuatePercentage($item_price,$item_cost);
                    /*$value['']*/
                    $trRows .= $this->load->view('jk_orders/parent_template', $value, TRUE); 
                }
            }
        }
        echo $trRows;
    ...
    function productFilter(){
        if( isset($_POST['search']) ) {
            ...
            $output = $this->db->like('item_code', $search,'both',false)
                              ->where($cond)
    ...
    function getChildProducts($style_id,$item_code){
        return $this->jk_model->getChildProStyleName( $item_code,$style_id );
    }
    ...
    function childProductStyle($item_code='', $style_id='', $parent_id='', $data_row_count=''){
        ...
        $data['jk_product'] = $this->jk_model->getChildProStyleName( $item_code,$style_id );
        if ( $data['jk_product'] ) { 
           ...
           echo $trRows;
            ...
            $varient = $this->jk_model->getProductFindByVariants($item_code,$style_id)[0];
            $data['jk_product'] = $this->jk_model->getChildProStyleName( $varient['item_code'],$style_id );
            if ( $data['jk_product'] ) { 
               $data['parent_id'] = array('parent_id' => $parent_id );
               $data['data_row_count'] = array('data_row_count' => $data_row_count );
               $trRows =  $this->load->view('jk_orders/child_template', $data, TRUE);
    ...
    function subChildProductStyle($item_code='', $style_id='', $parent_id='', $data_row_count=''){
        ...
        if ( $data['jk_product'] ) { 
           ...
           $trRows =  $this->load->view('jk_orders/subchild_template', $data, TRUE);
           echo $trRows;
        }else{
            $varient = $this->jk_model->getProductFindByVariants($item_code,$style_id)[0];
            if($varient){
    ...
    function productByStyleName(){
        ...
        if( isset($_POST['productId']) ){
           $proData = $this->jk_model->getProductById($_POST['productId']);
           if ($proData) {
               if ( $_POST['assemble_value'] == '1' ) {

                    $priceData = array( 
                            'item_cost'  => decimalValue($proData['item_cost_unassembled_with_tariff']), 
                    ...
                    $priceData = array(
                                    ...
                                    'item_price' => decimalValue($proData['assembled_item_cost'])
                                );
                } 
    ...
    function calcuateAmountPer($total_amount,$paid_amount){
         return decimalValue(( $paid_amount / $total_amount) * 100);          
    }
    ...
    function addCustomProduct($id = ''){
        ...
        if( $this->input->post() ){
            ...
            if ($item_code) {
                $orderMeta = array(
                    ...
                    'assembled_item_cost'  => $item_price,
                    'item_description'     => $description,
                );
                echo $this->jk_model->addCustomProduct($orderMeta);
            }else{
                echo "error";
            }
            die;
        ...
        $data['order_view'] = $this->jk_model->findWhere( 'pk_order', array( 'id' => $id ), FALSE, array( '*' ) );
        $data['vendor']     = $this->jk_model->fetch_all('vendor');
        $data['jk_style']   = $this->jk_model->fetch_all('jk_style');
        $this->load->view('jk_orders/custom_product',$data);
    }

    /* end new function after 21/11 */
    ...
    public function styleByVendor() {
        $vID = $_POST['v_id'];
        $styles = $this->db->where( array( 'vendor_id' => $vID ) )->get('jk_style')->result();
    ...
    function calcuateProQty(){
        if ( isset($_POST) ) {
            ...
            $data = $this->jk_model->getProductById( $proID,'product');


             if ( $_POST['assemble_value'] == 1 ) {
                $item_cost = $data['item_cost_unassembled_with_tariff'];
                $item_price = $data['unassembled_item_cost'];
            }else{
               $item_cost  = $data['item_assembly_cost'];
               $item_price = $data['assembled_item_cost'];
            }
    ...
    function quote($id)
    {
        ...
        $data['order_view'] = $this->jk_model->findWhere( 'pk_order', array( 'id' => $id ), FALSE, array( '*' ) );
        $data['pro_view'] = $this->jk_model->getJkProdyctByOrderId($id);
        $data['product_order_child'] = $this->jk_model->getJkProdyctChildByOrderId($id);
        $data['product_order_sub_child'] = $this->jk_model->getJkProdyctSubChildByOrderId($id);        

        $this->load->view('index', $data);
        $this->template->title('Order View');
        $this->template
                ->set_layout('inner')
                ->build('jk_orders/quote', $data);        
    ...
    {   
        ...
        $data['payment_data'] = $this->jk_model->findWhere( 'customer_payment', array( 'order_id' => $id ), TRUE, array( 'payment_amount' ) );
        $this->load->view('index', $data);
        $this->template->title('Order View');
        $this->template
                ->set_layout('inner')
                ->build('jk_orders/contract', $data);        
    ...
    function purchase_order($id)
    {
        ...
        $data['my_vendor']  = $this->jk_model->findWhere( 'vendor', array( 'id' => $vid ), FALSE, array( '*' ) );
        
        $this->load->view('index', $data);
        $this->template->title('Order View');
        $this->template
                ->set_layout('inner')
                ->build('jk_orders/purchase_order', $data);        
    ...
    {
        $data = array();
        $this->load->view('jk_orders/load_script', $data);
    }

    function load_css()
    {
        $data = array();
        $this->load->view('jk_orders/load_css', $data);
    }
}