...
class FabOrders extends MX_Controller
{
    ...
    function edit($id){
        ...
        $data['vendor']       = $this->fab_model->fetch_all('vendor');
    ...
    function productFilter(){ 
        if( isset($_POST['search']) ){
            ...
            foreach ($output as $row){
                $options .= "<li class='child_item' data-catid='".$row['id']."'><b>".$row['item_code'].$row['item_description']." ".$width.$height.$depth."</b></li>";
            }
...
/* ---------------------------  start calcuated copy function --------------------------- */
    ...
    function calcuateAllProductPrice(){
        if ( isset($_POST) ) {
            ...
            if ($_POST['perc_amount']) {
                $total_price -= $_POST['perc_amount'];
            }else if($_POST['price_perc']){
...
/* ---------------------------  end calcuated copy function ---------------------------  */
    ...
    function PoMSI(){
        ...
        $data['edit_order']   = $this->fab_model->getFabOrder($id);
        $data['fab_products'] = $this->fab_model->getFabProdyctByOrderId($id);
        $data['my_vendor']    = $this->fab_model->findWhere( 'vendor', array( 'id' => $vid ), FALSE, array( '*' ) );
        $this->template->title('Fab Edit');
        $this->template
                ->set_layout('inner')
