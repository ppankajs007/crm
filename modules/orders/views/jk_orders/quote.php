                                  ...
                                  $rawTotal = array();
                                    $total_sum = 0;
                                      foreach ($pro_view as $key => $value) {
                                        $parentProduct['data'] = $obj->quoteOrderForObject($order_view['assemble_value'],$value,'No');
                                        $rawTotal[] = $parentProduct['data']['totalprice'];
                                        $this->view( 'jk_orders/tr__template_quote',$parentProduct);
                                        if ( !empty($product_order_child) ) {
                                              foreach ($product_order_child as $key => $values) {
                                                  if( $value->POProId == $values->pro_order_id  ){ 

                                                      $childProduct['data'] = $obj->quoteOrderForObject($order_view['assemble_value'],$values,'No');

                                                      $childProduct['data']['child'] = 1;

                                                      $rawTotal[] = $childProduct['data']['totalprice'];

                                                      $this->view( 'jk_orders/tr__template_quote',$childProduct);
	                                                    ...
	                                                    if ( !empty($product_order_sub_child) ) {
                                                               ...
                                                               $subchildProduct['data'] = $obj->quoteOrderForObject($order_view['assemble_value'],$valuesub,'No');
                                                                $subchildProduct['data']['subchild '] = 1;
                                                      
                                                                $rawTotal[] = $subchildProduct['data']['totalprice'];

                                                                $this->view( 'jk_orders/tr__template_quote',$subchildProduct);
				                                                    }
				                                                }
				                                              }
				                                          }
               ...
               <div class="row">
                  ...
                  <div class="col-sm-6">
                     <div class="float-right">
                      <p><b>Subtotal:</b> <span class="float-right">$<?= decimalValue($total_sum,2); ?></span></p>
                       <?php if(!empty($order_view['discount_per']) && $order_view['discount_per'] != 0 ){
                            $discount_per = $order_view['discount_per'];
                            $discount     = decimalValue( $total_sum*$order_view['discount_per']/100 );
                        ...
                        <p><b>Total:</b> <span class="float-right"> &nbsp;&nbsp;&nbsp; 
                           $<?php echo decimalValue($total_sum - $discount); ?></span>
                        </p>
                        <h3></h3>
                     </div>
                     </div>
                     <div class="col-md-12">
                        <h6 style="text-align: center;" >Must add sales tax and delivery.</h6>
                      </div>
