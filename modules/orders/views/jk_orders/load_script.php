...
$globalMethods = array('index','edit', 'add', 'emails','product','quote_orders','jkOrders');
?>
...
<script>
	...
	function add_product(productId) {
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/jkOrders/productByCategory') ?>",
			type: "post",
...
<script>    
    jQuery( document ).on('change','#vendorName',function(){
		...
		if ( v_id == 3 ) {
			jQuery.ajax({
				url:"<?php echo base_url('orders/jkOrders/styleByVendor') ?>",
				type: "post",
				data:{ v_id: v_id },
				success:function(res){
    ...
    jQuery(document).on('click','.catDrop',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/jkOrders/categoryByStyle'); ?>",
			type:"post",
			data:{ style_id:sID},
			success:function(res){
    ...
    jQuery(document).on('click','#mainCats li',function(){
        ...
        jQuery('.resList ul').hide();
        jQuery.ajax({
			url:"<?php echo base_url('orders/jkOrders/categoryByProduct'); ?>",
			type:"post",
    ...
    jQuery(document).on('keyup input', '#searchPro', function (e) {
        ...
        if( srch.length > 1 ){
		    ...
		    jQuery('.resList ul#proRes').show();
            jQuery.ajax({
    			url:"<?php echo base_url('orders/jkOrders/productFilter'); ?>",
    			type:"post",
    ...
    function updateQty(uniquePro){
		...
		jQuery.ajax({
			async:false,
			url:"<?php echo base_url('orders/jkOrders/calcuateProQty'); ?>",
			type:"post",
    ...
    function calcuateAllProductPrice( updateDiscountAmt = true ){
		...
		var paid_amount  = jQuery( '#paidAmount' ).val();
 		...
 		jQuery.ajax({
			url:"<?php echo base_url('orders/jkOrders/calcuateAllProductPrice'); ?>",
			type:"post",
			data:{ 
	...
	jQuery( document ).on('keyup','.price_perc',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/jkOrders/calcuateAllProductByPerc'); ?>",
			type:"post",
			data:{ 
				total_price:total_sale_price, 
				perc:perc, 
	...
	jQuery( document ).on('keyup','.perc_amount',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/jkOrders/calcuateAllProductByAmt'); ?>",
			type:"post",
			data:{ 
				total_price : total_sale_price, 
				disc_amt    : disc_amt, 
	...
	jQuery( document ).on('keyup','.net_price',function(){
    	...
    	jQuery.ajax({
			url:"<?php echo base_url('orders/jkOrders/calcuateAllProductByNetPrice'); ?>",
			type:"post",
	...
	function calcuateTax(resale){
		...
		var resale = taxCostActive();
		jQuery.ajax({
			url: '<?php echo base_url('orders/jkOrders/calcuateTax'); ?>',
			type: 'post',
	...
	jQuery(document).on('keyup','.delivery_price_inp', function(){
		...
		var subtotal 	 = jQuery('.subtotal' ).val();
		jQuery.ajax({
			url: '<?php echo base_url('orders/jkOrders/customSum'); ?>',
			type: 'post',
	...
	jQuery( document ).on('click','.add_child',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/jkOrders/childProductStyle'); ?>",
			type:'post',
	...
	jQuery( document ).on('click','.add_subchild',function(){
		...
		jQuery.ajax({
			url:"<?php echo base_url('orders/jkOrders/subChildProductStyle'); ?>",
			type:'post',
	...
	function getSubChildProduct(productId,data_row_count,assemble_value){
		jQuery.ajax({
			url:"<?php echo base_url('orders/jkOrders/productByStyleName'); ?>",
			type:'post',
	...
	jQuery( document ).on('click','#importcustpro', function(e){
	    ...
	    setTimeout(function(){ 
			jQuery.ajax({
				url: "<?php echo base_url('orders/jkOrders/edit/'); ?>"+orderID,
			    type: "post",
			    data: formData,
			    contentType: false,
			    processData: false,
	...
	jQuery( document ).on('click','#save_custom_product',function(){
	    ...
	    jQuery( 'body' ).append( "<div class='loaderOverlay'><div class='inner'><img src='https://cdn-camp.mini-sites.net/Publish/53c7f5697dd94d829fcb41e20510e344/9acad7d360bd4f9a8b85ff13b1c61aac/src/images/loader01.gif' /></div></div>" );
		jQuery.ajax({
			url: "<?php echo base_url('orders/jkOrders/addCustomProduct'); ?>",
		    type: "post",
		    data: formData,
		    contentType: false,
		    processData: false,
