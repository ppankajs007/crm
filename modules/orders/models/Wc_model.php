...
class Wc_model extends MY_Model{ 
    ...
    function getCatByStyle($styleId){
        ...
        $cat = $this->findWhere( 'wc_product', array( 'style_id' => $styleId ), TRUE, array( '*' ) );
        foreach( $cat as $key => $value ){
            $cat_value[] = $value['category'];  
        }
        $uni_value = array_unique( $cat_value );
        foreach( $uni_value as $key => $value ){
            $data[] = $this->findWhere( 'wc_category', array( 'id' => $value ), FALSE, array(  'id','cat_name'  ) );
        }
        return $data;
    }
    ...
    function getCatByProduct(
        ...
        $product_table_name = 'wc_product';
        $cat_table_name = 'wc_category';
    ...
    function getProductByCat($productId){
        ...
        $sql = "SELECT jkpro.*, jkpro.id as jkpro_id, sy.*, sy.id as sid FROM wc_product jkpro LEFT JOIN wc_style sy on jkpro.style_id = sy.id WHERE jkpro.id = '$productId'";
    ...
    function getChildProStyleName($item_code,$style_id){
        $sql = "SELECT jkp.*,jkp.id as jkpid,sty.*,sty.id as styid FROM wc_product jkp LEFT JOIN wc_style sty on sty.id = jkp.style_id  WHERE parent LIKE '%".$item_code."%' AND style_id ='".$style_id."'";
    ...
    function getProductFilter($search,$cond){
        ...
        $output = $this->db->like('item_code', $search)->where($cond)->get('wc_product')->result_array();

        return $output;
    }

    function getProductById($proId){
        $sql = "SELECT jkp.*,jkp.id as jkpid,sty.*,sty.id as styid FROM wc_product jkp LEFT JOIN wc_style sty on sty.id = jkp.style_id  WHERE jkp.id ='".$proId."'";
        $rowCal = $this->custom_query($sql,false,'array')[0];
        if ( empty( $rowCal ) ) {
            logDbError( 'wc_model/getProductById', 'error');
        } 
    ...
    function getJkProdyctByOrderId($orderId){
        $sql = "SELECT jkPO.* ,jkP.*, jkStl.*,jkPO.id as POProId ,jkPO.style_id as POStyleId ,jkPO.item_code as item_code from wc_product_order as jkPO LEFT JOIN wc_product as jkP on jkP.id = jkPO.product_id LEFT JOIN wc_style as jkStl on jkStl.id = jkPO.style_id WHERE order_id = '$orderId'";
        
        $jkOrderPros = $this->custom_query($sql,FALSE,'array');
        if ( empty( $jkOrderPros ) ) {
            logDbError( 'wc_model/getJkProdyctByOrderId', 'error');
        }
        return $jkOrderPros;
    }

    function getJkProdyctChildByOrderId($orderId){
        $sql = "SELECT jkPOC.* ,jkP.*, jkStl.*,jkPOC.id as jkPOCid,jkP.id as jkPid , jkStl.id as jkStlid,jkPOC.style_id as POStyleId, jkPOC.item_code as item_code from wc_product_order_child as jkPOC LEFT JOIN wc_product as jkP on jkP.id = jkPOC.product_id LEFT JOIN wc_style as jkStl on jkStl.id = jkPOC.style_id WHERE order_id = '$orderId'";
        
        $jkOrderPros = $this->custom_query($sql,FALSE,'array');
        if ( empty( $jkOrderPros ) ) {
            logDbError( 'wc_model/getJkProdyctChildByOrderId', 'error');
        }
        return $jkOrderPros;
    }

     function getJkProdyctSubChildByOrderId($orderId){
        $sql = "SELECT jkPOC.* ,jkP.*, jkStl.*,jkPOC.id as jkPOCid,jkP.id as jkPid , jkStl.id as jkStlid, jkPOC.style as POStyleId from wc_product_order_subchild as jkPOC LEFT JOIN wc_product as jkP on jkP.id = jkPOC.product_id LEFT JOIN wc_style as jkStl on jkStl.id = jkPOC.style WHERE order_id = '$orderId'";
    ...
    function getJkOrder($orderId){
        ...
        if ( empty( $rowCal ) ) {
            logDbError( 'wc_model/getJkOrder', 'error');
        }else {
            $rowCal = $rowCal[0];
        }
        return $rowCal;
    ...
    function getProductFindByVariants($variants,$style_id){
        $sql = "SELECT jkp.*,jkp.id as jkpid,sty.*,sty.id as styid FROM wc_product jkp LEFT JOIN wc_style sty on sty.id = jkp.style_id  WHERE jkp.variants LIKE '%".$variants."%' AND style_id ='".$style_id."'";
        $data = $this->custom_query($sql,false,'array');
        return $data;
    }
    ...
    function saveNotExistProduct($data){
        ...
        if ( empty( $proRes ) ) {
            $this->db->insert( 'wc_product',$data);
    ...
    function saveJkOrder($orderID, $orderMeta){
        ...
        if( !empty( $orderMeta['product'] ) ){          
            ...
            $findData =  $this->countNumrows( 'wc_product_order', array( 'order_id' => $orderID ) );
            if( $findData == TRUE  ){
                $this->deleteWhere( 'wc_product_order',array( 'order_id' => $orderID ) );
            }
            $findDatasub =  $this->countNumrows( 'wc_product_order_child', array( 'order_id' => $orderID ) );
                if( $findDatasub == TRUE  ){
                    $this->deleteWhere( 'wc_product_order_child',array( 'order_id' => $orderID ) );
                }

            $findDatasubchild =  $this->countNumrows( 'wc_product_order_subchild', array( 'order_id' => $orderID ) );
            if( $findDatasubchild == TRUE  ){
                $this->deleteWhere( 'wc_product_order_subchild',array( 'order_id' => $orderID ) );
            } 
            foreach ($orderMeta['product'] as $key => $value) {
                                    
                    $sql = "INSERT INTO wc_product_order ( order_id, product_id,item_code, style_id,description,descriptionII, qty,price,u_price,total_price,final_cost) 
                ...
                if( isset($value['child_product']) ){
                    foreach ($value['child_product'] as $key => $values) {
                        ...
                        if( !empty($values['product_id'] ) ){
                            ...
                            $sqlsub = "INSERT INTO wc_product_order_child ( order_id, pro_order_id, qty, item_code, style_id, description,descriptionII,price,u_price,product_id,pro_parent_id,total_price,final_cost) 
                        ...
                        if( isset($values['subchild']) ){
                            ...
                            foreach ($values['subchild'] as $key => $subvalues) {
                                if( !empty($subvalues['product_id'] ) ){
                                    $sqlsubchild = "INSERT INTO wc_product_order_subchild ( order_id, pro_order_id, qty, item, style, description,descriptionII, price,u_price,product_id,total_price,final_cost,pro_parent_id) 
    ...
    function getProductDetails($orderID){
        ...
        $proRes = $this->db->select('pk.*,p_o.*,pk.id as pkid,pk.item_code as pkIN,pk.item_description as pkIdes,st.*,p_o.qty,st.style_name as sn')
                ->from('wc_product_order p_o')->join('wc_product pk', 'pk.id = p_o.product_id')->join('wc_style st','st.id = pk.style_id')->where( 'order_id',$orderID )->get()->result();
            //echo $this->db->last_query();die;

            if ( empty( $proRes ) ) {
                logDbError( 'wc_model/getProductDetails', 'error');
            }else {
                return $proRes;
            }
    ...
    function getChildProductQuote($id){
        ...
        $sqldata = "SELECT p_o_c.*,p_o_c.id as p_o_cid, pkpro.*,pkpro.id as pkpro_id,s.style_name as sn,s.style_code as sc FROM wc_product_order_child p_o_c LEFT JOIN wc_product pkpro on p_o_c.product_id = pkpro.id LEFT JOIN wc_style s on s.id = p_o_c.style_id WHERE order_id = '$id' ORDER BY pkpro.item_code";
        $data = $this->custom_query( $sqldata, false, 'object' );

        if( empty( $data ) ){
                logDbError( 'wc_model/getChildProductQuote', 'error');
        }else{
            return $data;
        }
    ...
    function getSubChildProductQuote($id){
        ...
        $sqlChildData = "SELECT p_o_sc.*,p_o_sc.id as p_o_scid, pkpro.*,pkpro.id as pkpro_id,s.style_name as sn,s.style_code as sc FROM wc_product_order_subchild p_o_sc LEFT JOIN wc_product pkpro on p_o_sc.product_id = pkpro.id LEFT JOIN wc_style s on s.id = p_o_sc.style WHERE order_id = '$id'";
        $data = $this->custom_query( $sqlChildData, false, 'object' );

        if( empty( $data ) ){
                 logDbError( 'wc_model/getSubChildProductQuote', 'error');
        }else{
            return $data;
        }
