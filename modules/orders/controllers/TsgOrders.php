...
class TsgOrders extends MX_Controller
{
    ...
    function edit($id){
        ...
        if( $this->input->post() ){
                            ...
                            if ($count > 1) {
                                ...
                                if ( $pro_Id != "" ) {
                                      ...
                                      if( $edit_order['assemble_value'] == 1 ){
                                        ...
                                        $cabinet_cost = decimalValue($pro_Id->unassembled_retail_item_price);
                                      } else{
                                        $item_cost    = decimalValue($pro_Id->cabinet_assembly_price);
                                        $cabinet_cost = decimalValue($pro_Id->assembled_retail_item_price);
                                      }
                                      ...
                                      $proDode = array( 'product_id' => $pro_Id->pkpro_id, 'qty' => $qty, 'description' => '','descriptionII' => '','item_code' => $itmCode,'style_id' => $styid,'price'=> '','u_price'=> '','total_price' => $cabinet_cost,'final_cost' => $item_cost,'item_cost' => $item_cost,'cabinaet_assemb_c' => $cabinet_cost ,'is_parent' => $is_parent,'assembled_retail_item_price' => $pro_Id->assembled_retail_item_price, 'parent_id' => $setParent);
                                        ...
                                        $varn_Id = $this->tsg_model->getProductFindByVariants($itmCode,$styid)[0];
                                        ...
                                        if ( !empty($varn_Id) ) {
                                            ...
                                            if( $edit_order['assemble_value'] == 1 ){
                                                $item_cost    = decimalValue($varn_Id['item_cost']);
                                                $cabinet_cost = decimalValue($varn_Id['unassembled_retail_item_price']);
                                            } else{
                                                $item_cost    = decimalValue($varn_Id['cabinet_assembly_price']);
                                                $cabinet_cost = decimalValue($varn_Id['assembled_retail_item_price']);
                                            }
                                        	...
                                        	$not_exist_product = [ 
                                                ...
                                                'item_cost' => '9999',
                                                'Assembly'                      => '9999',
                                                'unassembled_retail_item_price' => '9999',
                                                'cabinet_assembly_price'        => '9999',
                                                'assembled_retail_item_price'   => '9999',
                                                'assembly_retail_price'         => '9999',
                                                'vendor_id' => $this->session->userdata('vendor_id'), 
                                                'not_exist' => 'Yes' 
                                            ];

                                        	$not_exist_pro_id = $this->tsg_model->saveNotExistProduct($not_exist_product);

                                            $proDode = array( 'product_id' => $not_exist_pro_id, 'qty' => '1', 'item_code' => $itmCode,'style_id' => $styid,'description' => '','descriptionII' => '','price'=> '','u_price'=> '','item_cost' => '9999','cabinaet_assemb_c' => '9999','total_price' => '9999','final_cost' => '9999', 'is_parent' => $is_parent, 'parent_id' => $setParent, 'not_exist' => 'Yes' );
            ...
            $this->tsg_model->saveJkOrder( $id, $sArr );
            log_orders( 
                $id, //order_id 
        ...
        $data['edit_order']            = $this->tsg_model->getJkOrder($id);
        $data['vendor']                = multiArrSort($this->tsg_model->fetch_all('vendor'),'name');
        $data['jk_style']              = $this->tsg_model->fetch_all('style');
        $data['jk_products']           = $this->tsg_model->getJkProdyctByOrderId($id);
        $data['jk_products_child']     = $this->tsg_model->getJkProdyctChildByOrderId($id);
        $data['jk_products_subchild']  = $this->tsg_model->getJkProdyctSubChildByOrderId($id);
        $data['jkClass']               = $this;

        $this->template->title('Edit');
        $this->template
                ->set_layout('inner')
                ->build('tsg_orders/tsg_edit', $data);
    }
    ...
    public function categoryByStyle() {
        ...
        $allCats = $this->tsg_model->getCatByStyle($styleID);
        $options = "";
    ...
    public function categoryByProduct(){
        ...
        $rawRes = $this->tsg_model->getCatByProduct($cat_id,$cat_type,$p_style,$res_status,$mainCat,$childCat,$subCat, $subSubCat);
        $options = "";
        $dbRes = $rawRes['result'];
    ...
    function productByCategory()
    {
        ...
        if ( isset( $_POST['productId'] ) ) {
            ...
            $dbRes     = $this->tsg_model->getProductByCat($productId);
            if ($dbRes['status'] == 'success') {
                foreach ($dbRes['rows'] as $key => $value) {

                    if ( $_POST['assemble_val'] == '1' ) {
                            $item_cost = decimalValue($value['item_cost']);
                            $item_price = decimalValue($value['unassembled_retail_item_price']);
                    }else{
                        $item_cost  = decimalValue($value['cabinet_assembly_price']);
                        $item_price = decimalValue($value['assembled_retail_item_price']);
                    }
                    ...
                    $trRows .= $this->load->view('tsg_orders/parent_template', $value, TRUE); 
                }
            }
        }
        echo $trRows;
    ...
    function getChildProducts($style_id,$item_code){
        return $this->tsg_model->getChildProStyleName( $item_code,$style_id );
    }
    ...
    function childProductStyle($item_code='', $style_id='', $parent_id='', $data_row_count=''){
        if ( isset($_POST) ) {
            ...
            $data['jk_product'] = $this->tsg_model->getChildProStyleName( $item_code,$style_id );
            if ( $data['jk_product'] ) { 
               $data['parent_id'] = array('parent_id' => $parent_id );
               $data['data_row_count'] = array('data_row_count' => $data_row_count );
               $trRows =  $this->load->view('tsg_orders/child_template', $data, TRUE);
               echo $trRows;
            }else{
                $varient = $this->tsg_model->getProductFindByVariants($item_code,$style_id)[0];
                if( $varient ){
                    $data['jk_product'] = $this->tsg_model->getChildProStyleName( $varient['Item_Code'],$style_id );
                    if ( $data['jk_product'] ) { 
                       $data['parent_id'] = array('parent_id' => $parent_id );
                       $data['data_row_count'] = array('data_row_count' => $data_row_count );
                       $trRows =  $this->load->view('tsg_orders/child_template', $data, TRUE);
    ...
    function subChildProductStyle($item_code='', $style_id='', $parent_id='', $data_row_count=''){
        ...
        if ( $data['jk_product'] ) { 
           ...
           $trRows =  $this->load->view('tsg_orders/subchild_template', $data, TRUE);
           echo $trRows;
        }else{
            $varient = $this->tsg_model->getProductFindByVariants($item_code,$style_id)[0];
            if($varient){
    ...
    function productByStyleName(){
        ...
        if( isset($_POST['productId']) ){
           $proData = $this->tsg_model->getProductById($_POST['productId']);
           if ($proData) {
    ...
    function addCustomProduct($id = ''){
        ...
        if( $this->input->post() ){
            ...
            if ($item_code) {
                $orderMeta = array(
                    ...
                    'Item_Name'                     => $item_code,
                    'Item_Code'                     => $item_code,
                    'cabinet_assembly_price'        => $item_cost,
                    'assembled_retail_item_price'   => $item_price,
                    'Item_description'              => $description,
                    'custom_product'                => 'yes',
                    'user_id'                       => $this->session->userdata('user_id'),
                );
                echo $this->tsg_model->addCustomProduct($orderMeta);
            }else{
                echo "error";
            }
            die;
        ...
        $data['order_view'] = $this->tsg_model->findWhere( 'pk_order', array( 'id' => $id ), FALSE, array( '*' ) );
        $data['vendor']     = $this->tsg_model->fetch_all('vendor');
        $data['jk_style']   = $this->tsg_model->fetch_all('style');
        $this->load->view('tsg_orders/custom_product',$data);
    }
    ...
    function calcuateProQty(){
        if ( isset($_POST) ) {
            ...
            $data = $this->tsg_model->getProductById( $proID,'product');


            if ( $_POST['assemble_value'] == 1 ) {
                $item_cost = decimalValue($data['item_cost']);
                $item_price = decimalValue($data['unassembled_retail_item_price']);
            }else{
               $item_cost  = decimalValue($data['cabinet_assembly_price']);
               $item_price = decimalValue($data['assembled_retail_item_price']);
            }
    ...
    function quote($id)
    {
        ...
        $data['order_view'] = $this->tsg_model->findWhere( 'pk_order', array( 'id' => $id ), FALSE, array( '*' ) );
        $data['pro_view'] = $this->tsg_model->getProductDetails($id);
        $data['product_order_child'] = $this->tsg_model->getChildProductQuote($id);
        $data['product_order_sub_child'] = $this->tsg_model->getSubChildProductQuote($id);
        $data['obj'] = $this;       
    ...
    function quoteOrderForObject($assemble_value,$value,$PO_Quote){
        ...
        if( $assemble_value == 1 ){
              ...
              $cust_price = decimalValue($value->unassembled_retail_item_price);
        }else{
            $ted_price = decimalValue($value->cabinet_assembly_price);
            $cust_price = decimalValue($value->assembled_retail_item_price);
        }
    ...
    {   
        ...
        $data['order_view'] = $this->tsg_model->getJkOrder($id);
        ...
        $this->template
                ->set_layout('inner')
                ->build('tsg_orders/contract', $data);        
    ...
    function purchase_order($id)
    {
        ...
        $data['my_vendor']  = $this->tsg_model->findWhere( 'vendor', array( 'id' => $vid ), FALSE, array( '*' ) );
        $data['controllerThis'] = $this;
        ...
        $this->template
                ->set_layout('inner')
                ->build('tsg_orders/purchase_order', $data);        
    ...
    {
        $data = array();
        $this->load->view('tsg_orders/load_script', $data);
    }

    function load_css()
    {
        $data = array();
        $this->load->view('tsg_orders/load_css', $data);
    }
}