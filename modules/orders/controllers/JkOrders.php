...
class JkOrders extends MX_Controller
{
    ...
    function edit($id){
                                ...
                                if ( $pro_Id != "" ) {
                                      ...
                                      $proDode = array( 'product_id' => $pro_Id->pkpro_id, 'qty' => $qty, 'description' => '','descriptionII' => '','item_code' => $itmCode,'style_id' => $styid,'price'=> '','u_price'=> '','total_price' => $cabinet_cost,'final_cost' => $item_cost,'item_cost' => $item_cost,'cabinaet_assemb_c' => $cabinet_cost ,'is_parent' => $is_parent, 'parent_id' => $setParent);
                                        ...
                                        if ( !empty($varn_Id) ) {
                                            ...
                                            if( $edit_order['assemble_value'] == 1 ){
                                                $item_cost    = decimalValue($varn_Id['item_cost_unassembled_with_tariff']);
                                                $cabinet_cost = decimalValue($varn_Id['unassembled_item_cost']);
                                            } else{
                                                $item_cost    = decimalValue($varn_Id['item_assembly_cost']);
                                           ...
                                           $proDode = array( 'product_id' => $varn_Id['jkpid'], 'qty' => $qty, 'description' => '','descriptionII' => '','item_code' => $itmCode,'style_id' => $styid,'price'=> '','u_price'=> '','total_price' => $cabinet_cost,'final_cost' => $item_cost,'item_cost' => $item_cost,'cabinaet_assemb_c' => $cabinet_cost ,'is_parent' => $is_parent,
                                            ...
                                            $not_exist_product = [ 
                                                ...
                                                'style_id'                          => $styid , 
                                                'item_cost'                         => '9999',
                                                'item_cost_unassembled_with_tariff' => '9999',
        ...
        $data['vendor']                = multiArrSort($this->jk_model->fetch_all('vendor'),'name');
            ...
            if ($dbRes['status'] == 'success') {
                foreach ($dbRes['rows'] as $key => $value) {
                    ...
                    if ( $_POST['assemble_val'] == '1' ) {
                            $item_cost  = decimalValue($value['item_cost_unassembled_with_tariff']);
    ...
    function quote($id)
    {
        ...
        $data = array();
        $data['order_view'] = $this->jk_model->findWhere( 'pk_order', array( 'id' => $id ), FALSE, array( '*' ) );
        $data['pro_view'] = $this->jk_model->getProductDetails($id);
        $data['product_order_child'] = $this->jk_model->getChildProductQuote($id);
        $data['product_order_sub_child'] = $this->jk_model->getSubChildProductQuote($id);
        $data['obj'] = $this;       
    ...
    function quoteOrderForObject($assemble_value,$value,$PO_Quote){
        ...
        if( $assemble_value == 1 ){
              $ted_price = $value->item_cost_unassembled_with_tariff;
    ...
    {   
        ...
        $data['order_view'] = $this->jk_model->getJkOrder($id);
