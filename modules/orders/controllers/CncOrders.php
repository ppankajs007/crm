...
class CncOrders extends MX_Controller
{
    ...
    function edit($id){
        ...
        if( $this->input->post() ){
                        ...
                        foreach ($sheet->getRowIterator() as $row) { 
                            if ($count > 1) {
                                ...
                                $this->db->select('pkpro.*,pkpro.id as pkpro_id,pkpro.style_id as pkprost,sy.*,sy.id as sid');
                                         $this->db->from('cnc_product pkpro');
                                         $this->db->join('cnc_style sy', 'pkpro.style_id = sy.id', 'left');
                                ...
                                if ( $pro_Id != "" ) {
                                    ...
                                    if ( $edit_order['assemble_value'] == 1 ) {
                                        $item_cost    = decimalValue($pro_Id->item_cost_unassembled_tariff);
                                        $cabinet_cost = decimalValue($pro_Id->unassembled_item_cost);
                                    }else{
                                        $item_cost    = decimalValue($pro_Id->item_cost_assembled_tariff);
                                        $cabinet_cost = decimalValue($pro_Id->assembled_item_cost);
                                    }
                                      ...
                                      $proDode = array( 'product_id' => $pro_Id->pkpro_id, 'qty' => $qty, 'description' => '','descriptionII' => '','item_code' => $itmCode,'style_id' => $styid,'price'=> '','u_price'=> '','total_price' => $cabinet_cost,'final_cost' => $item_cost,'item_cost' => $item_cost,'is_parent' => $is_parent, 'parent_id' => $setParent);
                                        ...
                                        $varn_Id = $this->cnc_model->getProductFindByVariants($itmCode,$styid)[0];
                                        ...
                                        if ( !empty($varn_Id) ) {
                                            ...
                                            if ( $edit_order['assemble_value'] == 1 ) {
                                                ...
                                                $cabinet_cost = decimalValue($varn_Id['unassembled_item_cost']);
                                             }else{
                                                $item_cost    = decimalValue($varn_Id['item_cost_assembled_tariff']);
                                                $cabinet_cost = decimalValue($varn_Id['assembled_item_cost']);
                                            }
                                    ...
                                    /*pr( $proDode,1 );*/
                                           ...
                                           $proDode = array( 'product_id' => $varn_Id['jkpid'], 'qty' => $qty, 'description' => '','descriptionII' => '','item_code' => $itmCode,'style_id' => $styid,'price'=> '','u_price'=> '','total_price' => $cabinet_cost,'final_cost' => $item_cost,'item_cost' => $item_cost,'cabinaet_assemb_c' => $cabinet_cost ,'is_parent' => $is_parent,
                                            ...
                                            $not_exist_product = [ 
                                                ...
                                                'item_cost_assembled_tariff'                => '9999',
                                                'assembled_item_cost'               => '9999',
                                                'vendor_id'                         => $this->session->userdata('vendor_id'), 
                                                'not_exist'                         => 'Yes' 
                                            ];

                                            $not_exist_pro_id = $this->cnc_model->saveNotExistProduct($not_exist_product);
            ...
            $this->cnc_model->saveJkOrder( $id, $sArr );
            log_orders( 
                $id, //order_id 
        ...
        $data['edit_order']            = $this->cnc_model->getJkOrder($id);
        $data['vendor']                = multiArrSort($this->cnc_model->fetch_all('vendor'),'name');
        $data['jk_style']              = $this->cnc_model->fetch_all('cnc_style');
        $data['jk_products']           = $this->cnc_model->getJkProdyctByOrderId($id);
        $data['jk_products_child']     = $this->cnc_model->getJkProdyctChildByOrderId($id);
        $data['jk_products_subchild']  = $this->cnc_model->getJkProdyctSubChildByOrderId($id);
        $data['jkClass']               = $this;

        $this->template->title('CNC Edit');
        $this->template
                ->set_layout('inner')
                ->build('cnc_orders/cnc_edit', $data);
    }
    ...
    public function categoryByStyle() {
        ...
        $allCats = $this->cnc_model->getCatByStyle($styleID);
        $options = "";
    ...
    public function categoryByProduct(){
        ...
        $rawRes = $this->cnc_model->getCatByProduct($cat_id,$cat_type,$p_style,$res_status,$mainCat,$childCat,$subCat, $subSubCat);
        $options = "";
        $dbRes = $rawRes['result'];
    ...
    function productByCategory()
    {
        ...
        if ( isset( $_POST['productId'] ) ) {
            ...
            $dbRes     = $this->cnc_model->getProductByCat($productId);
            if ($dbRes['status'] == 'success') {
                foreach ($dbRes['rows'] as $key => $value) {
                    ...
                    if ( $_POST['assemble_val'] == '1' ) {
                        ...
                        $item_cost  = decimalValue($value['item_cost_assembled_tariff']);
                        $item_price = decimalValue($value['assembled_item_cost']) ;
                    }
                    ...
                    $trRows .= $this->load->view('cnc_orders/parent_template', $value, TRUE); 
                }
            }
        }
        echo $trRows;
    ...
    function getChildProducts($style_id,$item_code){
        return $this->cnc_model->getChildProStyleName( $item_code,$style_id );
    }
    ...
    function childProductStyle($item_code='', $style_id='', $parent_id='', $data_row_count=''){
        if ( isset($_POST) ) {
            ...
            $data['jk_product'] = $this->cnc_model->getChildProStyleName( $item_code,$style_id );
            if ( $data['jk_product'] ) { 
               $data['parent_id'] = array('parent_id' => $parent_id );
               $data['data_row_count'] = array('data_row_count' => $data_row_count );
               $trRows =  $this->load->view('cnc_orders/child_template', $data, TRUE);
               echo $trRows;
            }else{
                $varient = $this->cnc_model->getProductFindByVariants($item_code,$style_id)[0];
                if( $varient ){
                    $data['jk_product'] = $this->cnc_model->getChildProStyleName( $varient['item_code'],$style_id );
                    if ( $data['jk_product'] ) { 
                       $data['parent_id'] = array('parent_id' => $parent_id );
                       $data['data_row_count'] = array('data_row_count' => $data_row_count );
                       $trRows =  $this->load->view('cnc_orders/child_template', $data, TRUE);
    ...
    function subChildProductStyle($item_code='', $style_id='', $parent_id='', $data_row_count=''){
        ...
        if ( $data['jk_product'] ) { 
           ...
           $trRows =  $this->load->view('cnc_orders/subchild_template', $data, TRUE);
           echo $trRows;
        }else{
            $varient = $this->cnc_model->getProductFindByVariants($item_code,$style_id)[0];
            if($varient){
    ...
    function productByStyleName(){
        ...
        if( isset($_POST['productId']) ){
           $proData = $this->cnc_model->getProductById($_POST['productId']);
           if ($proData) {
               if ( $_POST['assemble_value'] == '1' ) {
                    ...
                    $priceData = array(
                        'item_cost'  => decimalValue($proData['item_cost_assembled_tariff']),
    ...
    function addCustomProduct($id = ''){
        ...
        if( $this->input->post() ){
            ...
            if ($item_code) {
                $orderMeta = array(
                    ...
                    'vendor_id'            => $this->session->userdata('vendor_id'),
                    'item_code'            => $item_code,
                    'item_cost_assembled_tariff'   => $item_cost,
                ...
                //pr( $orderMeta,1 );
                echo $this->cnc_model->addCustomProduct($orderMeta);
                log_orders( 
                    $order_id, //order_id 
                    $this->session->userdata('user_id'), 
                    'AddProduct(CncOrders)'
                );
            }else{
                echo "error";
            }
        ...
        $data['order_view'] = $this->cnc_model->findWhere( 'pk_order', array( 'id' => $id ), FALSE, array( '*' ) );
        $data['vendor']     = $this->cnc_model->fetch_all('vendor');
        $data['jk_style']   = $this->cnc_model->fetch_all('cnc_style');
        $this->load->view('cnc_orders/custom_product',$data);
    }
    ...
    public function styleByVendor() {
        $vID = $_POST['v_id'];
        $styles = $this->db->where( array( 'vendor_id' => $vID ) )->get('cnc_style')->result();
    ...
    function calcuateProQty(){
        if ( isset($_POST) ) {
            ...
            $data = $this->cnc_model->getProductById( $proID,'cnc_product');
            ...
            if ( $_POST['assemble_value'] == 1 ) {
               ...
               $item_cost  = decimalValue($data['item_cost_assembled_tariff']);
               $item_price = decimalValue($data['assembled_item_cost']);
            }
    ...
    function quote($id)
    {
        ...
        $data = array();
        $data['order_view'] = $this->cnc_model->findWhere( 'pk_order', array( 'id' => $id ), FALSE, array( '*' ) );
        $data['pro_view'] = $this->cnc_model->getProductDetails($id);
        $data['product_order_child'] = $this->cnc_model->getChildProductQuote($id);
        $data['product_order_sub_child'] = $this->cnc_model->getSubChildProductQuote($id);
        $data['obj'] = $this;       
    ...
    function quoteOrderForObject($assemble_value,$value,$PO_Quote){
        ...
        if( $assemble_value == 1 ){
              $ted_price = $value->item_cost_unassembled_tariff;
              $cust_price = $value->unassembled_item_cost;
        }else{
            $ted_price = $value->item_cost_assembled_tariff;
            $cust_price = $value->assembled_item_cost;
        }
    ...
    {   
        ...
        $data['order_view'] = $this->cnc_model->getJkOrder($id);
        ...
        $this->template
                ->set_layout('inner')
                ->build('cnc_orders/contract', $data);        
    ...
    function purchase_order($id)
    {
        ...
        $data['my_vendor']  = $this->cnc_model->findWhere( 'vendor', array( 'id' => $vid ), FALSE, array( '*' ) );
        $data['controllerThis'] = $this;
        ...
        $this->template
                ->set_layout('inner')
                ->build('cnc_orders/purchase_order', $data);        
    ...
    {
        $data = array();
        $this->load->view('cnc_orders/load_script', $data);
    }

    function load_css()
    {
        $data = array();
        $this->load->view('cnc_orders/load_css', $data);
    }
}