...
class WcOrders extends MX_Controller
{
    ...
    if( $this->input->post() ){
        ...
        if (!empty($_FILES['file_upload']['name'])) {
                    ...
                    foreach ($sheet->getRowIterator() as $row) { 
                        if ($count > 1) {
                            ...
                            $this->db->select('pkpro.*,pkpro.id as pkpro_id,pkpro.style_id as pkprost,sy.*,sy.id as sid');
                                     $this->db->from('wc_product pkpro');
                                     $this->db->join('wc_style sy', 'pkpro.style_id = sy.id', 'left');
                            ...
                            if ( $pro_Id != "" ) {
                                  ...
                                  if( empty( $pro_Id->depth ) || ( $pro_Id->depth =='NA' ) ) {$depth='';}else{$depth = $pro_Id->depth.'D';}
                                   ...
                                   if ( $_POST['assemble_val'] == 1 ) {
                                            $item_cost    = decimalValue($pro_Id->item_cost_unassembled_with_tariff);
                                            $cabinet_cost = decimalValue($pro_Id->unassembled_item_price);
                                  }else{
                                        $item_cost    = decimalValue($pro_Id->item_cost_assembled);
                                  ...
                                  $proDode = array( 'product_id' => $pro_Id->pkpro_id, 'qty' => $qty, 'description' => '','descriptionII' => '','item_code' => $itmCode,'style_id' => $styid,'price'=> '','u_price'=> '','total_price' => $cabinet_cost,'final_cost' => $item_cost,'item_cost' => $item_cost,'cabinaet_assemb_c' => $cabinet_cost ,'is_parent' => $is_parent,'assembled_retail_item_price' => $pro_Id->assembled_retail_item_price, 'parent_id' => $setParent);
                                    ...
                                    $varn_Id = $this->wc_model->getProductFindByVariants($itmCode,$styid)[0];
                                    ...
                                    if ( !empty($varn_Id) ) {
                                        ...
                                        if ( $_POST['assemble_val'] == 1 ) {
                                                $item_cost    = decimalValue($varn_Id['item_cost_unassembled_tariff']);
                                                $cabinet_cost = decimalValue($varn_Id['unassembled_item_price']);
                                          }else{
                                                $item_cost    = decimalValue($varn_Id['item_cost_assembled']);
                                                $cabinet_cost = decimalValue($varn_Id['assembled_item_price']);
                                          }
                                        ...
                                        $proDode = array( 'product_id' => $varn_Id['jkpid'], 'qty' => $qty, 'description' => '','descriptionII' => '','item_code' => $itmCode,'style_id' => $styid,'price'=> '','u_price'=> '','total_price' => $cabinet_cost,'final_cost' => $item_cost,'item_cost' => $item_cost,'cabinaet_assemb_c' => $cabinet_cost ,'is_parent' => $is_parent,'assembled_retail_item_price' => $varn_Id['assembled_retail_item_price'], 'parent_id' => $setParent);
                                    ...
                                    }elseif ( empty($varn_Id) && empty( $pro_Id )  ) {
                                        ...
                                        $not_exist_product = [ 
                                            ...
                                            'not_exist'                     => 'Yes', 
                                            'item_cost_unassembled_tariff'  => '9999',
                                            'unassembled_item_cost'         => '9999',
                                            'item_cost_assembled'           => '9999',
                                            'assembled_item_price'          => '9999',
                                        ];
                                        $not_exist_pro_id = $this->wc_model->saveNotExistProduct($not_exist_product);

                                        $proDode = array( 'product_id' => $not_exist_pro_id, 'qty' => '1', 'item_code' => $itmCode,'style_id' => $styid,'description' => '','descriptionII' => '','price'=> '','u_price'=> '','item_cost' => 0,'cabinaet_assemb_c' => 0,'total_price' => '','final_cost' => '', 'is_parent' => $is_parent, 'parent_id' => $setParent,'' => 0, 'not_exist' => 'Yes' );
            ...
            $sArr['product'] = $data;
            $sArr['is_import'] = 'yes';
            $finalProList = $this->searchForSameProduct($sArr['product']);

            if ($finalProList) {
                $sArr['product'] = $finalProList;
            }
        }else{
            $sArr['is_import'] = 'no';
        }
        $this->wc_model->saveJkOrder( $id, $sArr );
        log_orders( 
            $id, //order_id 
        ...
        $data['edit_order']   = $this->wc_model->getJkOrder($id);
        $data['vendor']       = multiArrSort($this->wc_model->fetch_all('vendor'),'name');
        $data['jk_style']     = $this->wc_model->fetch_all('wc_style');
        $data['jk_products']  = $this->wc_model->getJkProdyctByOrderId($id);
        $data['jk_products_child']  = $this->wc_model->getJkProdyctChildByOrderId($id);
        $data['jk_products_subchild']  = $this->wc_model->getJkProdyctSubChildByOrderId($id);
        $data['jkClass']  = $this;

        $this->template->title('Wc Edit');
        $this->template
                ->set_layout('inner')
                ->build('wc_orders/wc_edit', $data);
}

    public function searchForSameProduct($allItems){
        $newArr = array();
    ...
    public function categoryByStyle() {
        ...
        $allCats = $this->wc_model->getCatByStyle($styleID);
        $options = "";
    ...
    public function categoryByProduct(){
        ...
        $rawRes = $this->wc_model->getCatByProduct($cat_id,$cat_type,$p_style,$res_status,$mainCat,$childCat,$subCat, $subSubCat);
        $options = "";
        $dbRes = $rawRes['result'];
    ...
    function productByCategory()
    {
        ...
        if ( isset( $_POST['productId'] ) ) {
            ...
            $dbRes     = $this->wc_model->getProductByCat($productId);
            if ($dbRes['status'] == 'success') {
                foreach ($dbRes['rows'] as $key => $value) {

                    if ( $_POST['assemble_val'] == '1' ) {
                        $item_cost  = decimalValue($value['item_cost_unassembled_tariff']);
                        $item_price = decimalValue($value['unassembled_item_cost']) ;
                    }else{
                        $item_cost  = decimalValue($value['item_cost_assembled']);
                    ...
                    /*$value['']*/
                    $trRows .= $this->load->view('wc_orders/parent_template', $value, TRUE); 
                }
            }
        }
        echo $trRows;
    ...
    function getChildProducts($style_id,$item_code){
        return $this->wc_model->getChildProStyleName( $item_code,$style_id );
    }
    ...
    function childProductStyle($item_code='', $style_id='', $parent_id='', $data_row_count=''){
        if ( isset($_POST) ) {
            ...
            $data['jk_product'] = $this->wc_model->getChildProStyleName( $item_code,$style_id );
            if ( $data['jk_product'] ) { 
               $data['parent_id'] = array('parent_id' => $parent_id );
               $data['data_row_count'] = array('data_row_count' => $data_row_count );
               $trRows =  $this->load->view('wc_orders/child_template', $data, TRUE);
               echo $trRows;
            }else{
                $varient = $this->wc_model->getProductFindByVariants($item_code,$style_id)[0];
                $data['jk_product'] = $this->wc_model->getChildProStyleName( $varient['item_code'],$style_id );
                if( $varient ){
                    if ( $data['jk_product'] ) { 
                       $data['parent_id'] = array('parent_id' => $parent_id );
                       $data['data_row_count'] = array('data_row_count' => $data_row_count );
    ...
    function subChildProductStyle($item_code='', $style_id='', $parent_id='', $data_row_count=''){
        ...
        if ( $data['jk_product'] ) { 
           ...
           $trRows =  $this->load->view('wc_orders/subchild_template', $data, TRUE);
           echo $trRows;
        }else{
            $varient = $this->wc_model->getProductFindByVariants($item_code,$style_id)[0];
            if($varient){
    ...
    function productByStyleName(){
        ...
        if( isset($_POST['productId']) ){
           $proData = $this->wc_model->getProductById($_POST['productId']);
    ...
    function addCustomProduct($id = ''){
        ...
        if( $this->input->post() ){
            ...
            if ($item_code) {
                $orderMeta = array(
                    ...
                    'vendor_id'              => $this->session->userdata('vendor_id'),
                    'item_code'              => $item_code,
                    'item_cost_assembled'    => $item_cost,
                    'assembled_item_price'   => $item_price,
                    'item_description'       => $description,
                    'custom_product'         => 'yes',
                    'user_id'                => $this->session->userdata('user_id'),
                );
                echo $this->wc_model->addCustomProduct($orderMeta);
            }else{
                echo "error";
            }
            die;
        ...
        $data['order_view'] = $this->wc_model->findWhere( 'pk_order', array( 'id' => $id ), FALSE, array( '*' ) );
        $data['vendor']     = $this->wc_model->fetch_all('vendor');
        $data['jk_style']   = $this->wc_model->fetch_all('wc_style');
        $this->load->view('wc_orders/custom_product',$data);
    }
    ...
    function calcuateProQty(){
        if ( isset($_POST) ) {
            ...
            $data = $this->wc_model->getProductById( $proID,'wc_product');


            if ( $_POST['assemble_value'] == 1 ) {
                $item_cost = decimalValue($data['item_cost_unassembled_tariff']);
                $item_price = decimalValue($data['unassembled_item_cost']);
            }else{
               $item_cost  = decimalValue($data['item_cost_assembled']);
...
function quote($id)
    {
        ...
        $data = array();
        $data['order_view'] = $this->wc_model->findWhere( 'pk_order', array( 'id' => $id ), FALSE, array( '*' ) );
        $data['pro_view'] = $this->wc_model->getProductDetails($id);
        $data['product_order_child'] = $this->wc_model->getChildProductQuote($id);
        $data['product_order_sub_child'] = $this->wc_model->getSubChildProductQuote($id);
        $data['obj'] = $this;       
    ...
    {   
        ...
        $data['order_view'] = $this->wc_model->getJkOrder($id);
    ...
    function purchase_order($id)
    {
        ...
        $data['my_vendor']  = $this->wc_model->findWhere( 'vendor', array( 'id' => $vid ), FALSE, array( '*' ) );
        $data['controllerThis'] = $this;
        ...
        $this->template
                ->set_layout('inner')
                ->build('wc_orders/purchase_order', $data);        
    ...
    function quoteOrderForObject($assemble_value,$value,$PO_Quote){
        ...
        if( $assemble_value == 1 ){
              $ted_price = decimalValue($value->item_cost_unassembled_tariff);
              $cust_price = decimalValue($value->unassembled_item_cost);
        }else{
            $ted_price = decimalValue($value->item_cost_assembled);
            $cust_price = decimalValue($value->assembled_item_price);
        }
    ...
    {
        $data = array();
        $this->load->view('wc_orders/load_script', $data);
    }

    function load_css()
    {
        $data = array();
        $this->load->view('wc_orders/load_css', $data);
    }
}