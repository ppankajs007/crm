...
class Qualified extends MX_Controller 
{
  ...
  function getQualified(){
    ...
    $requestData= $_REQUEST;
      ...
      $sql.=" FROM leads l left join user_profiles p on l.assigned_to =p.user_id left join pw_pending_w pw ON pw.lead_id = l.id WHERE vendor_id = $vendor_id AND  l.lead_status ='5' ";*/
...
( SELECT SUM(`point`)+ (SELECT hoteness*2*10  FROM leads WHERE id = l.id) FROM `survey_qoptions` WHERE id IN (SELECT answer FROM `survey_answers` WHERE lead_id = l.id) ) AS hs, s.status AS int_status";
      $sql.=" FROM leads l left join user_profiles p on l.assigned_to =p.user_id left join pw_pending_w pw ON pw.lead_id = l.id left join lead_int_status s on l.qf_status=s.id WHERE l.vendor_id = '$vendor_id' AND  l.lead_status ='".lead_Qualified."' ";
      ...
      { 
          ...
          foreach($total_record as $row){
                ...
                if( !empty( $row["qf_dateRecieved"] ) ){ 
                        /*$Rec = $row["qf_dateRecieved"]; 
                        $Recieved = date("m/d/Y", strtotime($Rec)); */
                        $Recieved = crm_date($row["qf_dateRecieved"]);
                }else{ $Recieved = 'N/A'; }
                
                if( !empty( $row["qf_dateCompleted"] ) ){ 
                    //$Completed = date("m/d/Y", strtotime($row["qf_dateCompleted"])); 
               ...
               /* $surveyQus = $this->crm_model->findWhere('survey_answers', array('lead_id' => $row['lid']), true, array('answer') );
                $sum = 0;
...
// 	         $Pwdata = (array) App::get_row_by_where('pw_pending_w', array('lead_id'=>$lead_id) );
// 	            if(!empty($Pwdata['lead_id'])){
// 	                    $Pwid=$Pwdata['lead_id'];
// 	                    }else{
// 	                      $Pwid='';  
// 	                      }
...
// 				   'option_request'                   => $this->input->post('option_request'),
...
// 				if( !$pwid = App::save_data( 'pw_pending_w', $post_data ) )
//                 {
//                     $this->session->set_flashdata('response_status', 'error');
//                     $this->session->set_flashdata('message', 'Qualified Lead didn\'t save, Try again!');
//                     redirect('/crm/Qualified');
//                 }else{
                    
//                     $post_data['success'] = array();
//                     $this->session->set_flashdata('response_status','success'); 
//                     $this->session->set_flashdata('message','Qualified Lead  Added successfully');
//                      redirect('/crm/Qualified');
//                  }
// 	    	}
// 	       }else{
...
//         $this->template->title(' Leads');
//         $this->template->set_layout('inner')->build('Qualified/QualifiedLeads/QualifiedForm', $data);
	             ...
	             if(isset($_POST)){ 
	           	...
	           	$post_data = array(
        				   ...
        				   'refrigerator_location_be_moved'   => $this->input->post('refrigerator_location_be_moved'),
                            ...
                            'activity'=> 'Create Kitchan Details Qualified : Leads '
                          );
...
//       $id = $this->uri->segment(4);
//           $data = array();
//           $data['file_data'] = $this->crm_model->findWhere('files', array('field_id' => $id,'module_name' => 'leads'  ), array('*') );
        ...
        $error = array('error' => $this->upload->display_errors());
          ...
          $data = array( 
            ...
            $activaty = array( 
                            ...
                            'activity'=> 'Create a File Qualified : '.$data['module_name']
                          );
   ...
   function notes($id){   
        ...
        if ( $this->input->post('lead_id') ) {    
            ...
            $activaty = array( 
                    ...
                    'module'  => 'leads',
                    'field_id' => $match['id'],
                    'activity'=> 'Created a Note Qualified : '.$query->first_name
                  );
            $this->session->set_flashdata('response_status','success'); 
                    ...
                    $curr_time            = date('h:i:sa');
                   	...
                   	if( !$hold_id = App::save_data( 'hold_on_desgin', $post_data ) )
                        ...
                        $match = array (
                         ...
                         $phdata = array(
                                  'qf_status'         => int_lead_Hold_On_Design,
...
/*	function add_st(){
	     ...
	     $vl=$_POST['lostValue'];
	     $id= $_POST['leads_id'];
	    // if( $vl=='Need More Info to Start' || $vl=='Revision Required'){
	      $match = array (
                'id' => $id,
            );
