...
class MRLeads extends MX_Controller 
{
     ...
     $data = array();
      ...
      if ( $this->input->post('lead_id') ) {
        ...
        $match=array(
         'id' => $this->input->post('lead_id'),
          ...
          $post_data = array(
                ...
                if($cmr_status != 'Measurement Scheduled'){
            ...
            $db_activity_id = log_activity('lead_activity',$id,$get_lead_status="",$new_lead_status="",'Update Lead Measurement','activity_record');
           ...
           log_message('error',serialize($post_adding));
                ...
                App::update('leads',$match,$post_data);
                  $activaty = array( 
                            ...
                            'module'  => 'leads',
                            'field_id' =>$match['id'],
                            'activity'=> 'Measurement  Leads  : Edit'
                          );
        ...
        $data['lead_statuss'] = $result->result('array');
             //lead status
         $data['lead_statuss'] = $this->crm_model->custom_query("SELECT * FROM status_lead WHERE id IN ( ".lead_Presentation_Lead.", ".lead_Qualified.", ".lead_Lost_sale.", ".lead_Measurement_Required." )",false,'array');
            //internal status
  ...
  function getMRleads(){
    ...
    $sql.=" FROM leads l left join user_profiles p on l.assigned_to = p.user_id left join lead_int_status s on l.mr_status = s.id WHERE vendor_id = $vendor_id  AND l.lead_status = '".lead_Measurement_Required."' ";
     ...
     // pr($totalData);
      ...
      if( !empty($requestData['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
               ...
               $sql.=" OR l.mr_surveysent LIKE '%".$requestData['search']['value']."%' ";
               $sql.=" OR l.mr_surveycompleted LIKE '%".$requestData['search']['value']."%' ";
      ...
      { 
          ...
          foreach($total_record as $row){
            ...
            //   } 
              ...
              // $mr_msdate = date('m/d/Y', $row['mr_msdate']);
...
//           $data = array();
//           $data['file_data'] = $this->crm_model->findWhere('files', array('field_id' => $id,'module_name' => 'PWleads'  ), array('*') );
        ...
        $error = array('error' => $this->upload->display_errors());
          ...
          $data = array( 
            ...
            $activaty = array( 
                            ...
                            'activity'=> 'Created a file Measurement  : '.$data['module_name']
                          );
   ...
   function notes($id){   
        ...
        if ( $this->input->post('lead_id') ) {    
            ...
            $activaty = array( 
                    ...
                    'module'  => 'leads',
                    'field_id' => $match['id'],
                    'activity'=> 'Created a Note Measurement  : '.$query->first_name
                  );
    ...
    function pw_form(){
		...
		$db_activity_id = log_activity('lead_activity',$id,$get_lead_status="",$new_lead_status="",'Add Kitchen Detial From Measurement Lead','activity_record');
           $post_data2 = array( 'activity_id' => $db_activity_id );
           $post_adding = array_merge( $post_data, $post_data2 );
           log_message('error',serialize($post_adding));
	             ...
	             if(isset($_POST)){ 
	           	...
	           	$post_data = array(
        	               ...
        	               $activaty = array( 
                            ...
                            'module'  => 'leads',
                            'field_id' =>$match['lead_id'],
                            'activity'=> 'Create kitchen Details Measurement : Leads'
                          );
         ...
         $db_activity_id = log_activity('lead_activity',$id,$get_lead_status="",$new_lead_status="",'Add Kitchen Detial From Measurement Lead','activity_record');
           ...
           log_message('error',serialize($post_adding));
                           App::LogActivity($activaty);
        	                 $post_data['success'] = array();
                             $this->session->set_flashdata('response_status','success'); 
                             $this->session->set_flashdata('message','Measurement Kitchen Details Update successfully');
                              redirect('crm/MRLeads/pw_form/'. $id);
          }
	      }
	    }
       ...
       $ltdata= (array) App::get_row_by_where('task', array('id'=>$lead_id) );
         ...
         //$lead_id = $this->input->post('lead_id');
          ...
          $status = $this->input->post('status');
           ...
           $db_activity_id = log_activity('lead_activity',$id,$get_lead_status="",$new_lead_status="",'Add Task','activity_record');
           $post_data2 = array( 'activity_id' => $db_activity_id );
           $post_adding = array_merge( $data, $post_data2 );
           log_message('error',serialize($post_adding));
