...
class Tank_auth
{
	 ...
	 * @return	bool
	 */
	function login($login, $password, $remember, $vendor_id, $login_by_username, $login_by_email)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->Users->$get_user_func($login))) {	// login ok
			    ...
			    if ( ($hasher->CheckPassword($password, $user->password)) && ($vendor_id == $user->vendor_id) ){		// password ok
	 ...
	 * @return	array
	 */
	function create_user($username, $name, $email, $role, $department, $password, $email_activation,$vendor_id)
	{
